...
" Every mount-cap goal depends on the buffer-cap class. Per default, no buffer-cap goal is grounded. "
	(declare (salience (+ ?*SALIENCE-GOAL-EXECUTABLE-CHECK* 1))) ;needs to be higher than SALIENCE-GOAL-EXECUTABLE-CHECK
...
(defrule goal-dependencies-mount-cap-buffer-cap-executable ;output is clear
" Even if the CS is not buffered, mount-cap can be executable if the output is free and 
  there is a buffer-cap goal running for the CS. "
	...
	=>
	(printout t "MOUNT-CAP Goal executable for " ?robot " depending on " ?dependency-class " goal " ?dependency-goal-id crlf)
	...
	(if (eq ?dependency-class BUFFER-CAP)
		then (do-for-fact ((?instruct-goal goal) (?instruct-da dependency-assignment)) 
			 ...
			 (printout t "MOUNT-CAP Goal executable for " ?robot " depending on INSTRUCT-CS-BUFFER-CAP goal " crlf)
	)
)
