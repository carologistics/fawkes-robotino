...
(defrule goal-expander-transport
  ?g <- (goal (id ?goal-id) (parent ?parent-id) (mode SELECTED) (class TRANSPORT)
              (params wp ?wp mps-from ?mps-from mps-from-side ?mps-from-side mps-to ?mps-to))
  (wm-fact (key domain fact entered-field args? r ?robot))
...
(defrule goal-expander-fetch-base
  (declare (salience ?*SALIENCE-GOAL-EXPAND*))
  ?g <- (goal (id ?goal-id) (parent ?parent-id) (mode SELECTED) (class FETCH-BASE) (params base-color ?base-color spawned-wp ?spawned-wp))
...
=>
  (printout t "Expanding FETCH-BASE with " ?robot " " ?base-color " " ?bs crlf)
...
(defrule goal-expander-mount-cap
  (declare (salience ?*SALIENCE-GOAL-EXPAND*))
  ?g <- (goal (id ?goal-id) (parent ?parent-id) (mode SELECTED) (class MOUNT-CAP) (params spawned-wp ?spawned-wp mps ?cs cap-color ?cap-color))
  (not (goal (parent ?parent-id) (class DISCARD-BASE)))
  (wm-fact (key domain fact entered-field args? r ?robot))
  (not (goal (meta r ?robot)))
  (wm-fact (key domain fact wp-at args? wp ?spawned-wp m ?cs side INPUT))
=>
  (printout t "Expanding MOUNT-CAP with " ?robot " " ?spawned-wp " " ?cs crlf)
  ...
  (assert
    ...
    (plan-action (id 2) (plan-id ?plan-id) (goal-id ?goal-id)
              ...
              (skiller (remote-skiller ?robot))
              (param-values ?cs ?spawned-wp ?cap-color))
  )
  (modify ?g (mode EXPANDED) (meta r ?robot))
)
...
=>
  ...
  (if (eq ?complexity C0) then
    (assert (plan-action (id 6) (plan-id ?plan-id) (goal-id ?goal-id)
...
(defrule goal-expander-mount-r1p
  (declare (salience ?*SALIENCE-GOAL-EXPAND*))
  ...
  (not (printed ?rs ?ring1-color ?bases-needed))
=> 
  (printout t "rs-ring-spec " ?rs " " ?ring1-color " " ?bases-needed crlf)
  (assert (printed ?rs ?ring1-color ?bases-needed))
)
