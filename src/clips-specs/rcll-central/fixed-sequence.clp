...
(defrule goal-expander-produce-c1-get-base-wait
 ?p <- (goal (mode DISPATCHED) (id ?parent) (class PRODUCE-C1) (params order ?order bs-color ?base-color ring1-color ?ring1-color cs-color ?cap-color wp ?wp bs ?bs cs ?cs ds ?ds rs ?rs))
 ?g <- (goal (id ?goal-id) (parent ?parent) (class GET-BASE-WAIT) (mode SELECTED))
 (wm-fact (key domain fact entered-field args? r ?robot))
 (wm-fact (key domain fact at args? r ?robot m ?curr-location side ?curr-side))
 (wm-fact (key domain fact can-hold args? r ?robot))
 (not(goal (parent ?parent) (params robot ?robot)))
 =>
      (bind ?planid (sym-cat PRODUCE-C1-GET-BASE-WAIT-PLAN- (gensym*)))
      (assert
        (plan (id ?planid) (goal-id ?goal-id))
        (plan-action (id 1) (plan-id ?planid) (goal-id ?goal-id)
              (action-name spawn-wp)
...
(defrule goal-expander-produce-c1-buffer-cs
 ?p <- (goal (mode DISPATCHED) (id ?parent) (class PRODUCE-C1) (params order ?order bs-color ?base-color ring1-color ?ring1-color cs-color ?cap-color wp ?wp bs ?bs cs ?cs ds ?ds rs ?rs))
 ?g <- (goal (id ?goal-id) (parent ?parent) (class BUFFER-CS) (mode SELECTED))
 ...
 (wm-fact (key domain fact wp-cap-color args? wp ?cc col ?cap-color))
 =>
      (bind ?planid (sym-cat PRODUCE-C1-BUFFER-CS-PLAN- (gensym*)))
      (assert
        (plan (id ?planid) (goal-id ?goal-id))
        (plan-action (id 1) (plan-id ?planid) (goal-id ?goal-id)
              (action-name go-wait)
...
(defrule goal-expander-produce-c1-buffer-rs
 ?p <- (goal (mode DISPATCHED) (id ?parent) (class PRODUCE-C1) (params order ?order bs-color ?base-color ring1-color ?ring1-color cs-color ?cap-color wp ?wp bs ?bs cs ?cs ds ?ds rs ?rs))
 ?g <- (goal (id ?goal-id) (parent ?parent) (class BUFFER-RS) (mode SELECTED))
 (wm-fact (key domain fact entered-field args? r ?robot))
 (wm-fact (key domain fact at args? r ?robot m ?curr-location side ?curr-side))
 (wm-fact (key domain fact can-hold args? r ?robot))
 (not(goal (parent ?parent) (params robot ?robot)))
 =>
      (bind ?planid (sym-cat PRODUCE-C1-BUFFER-RS-PLAN- (gensym*)))
      (assert
        (plan (id ?planid) (goal-id ?goal-id))
        (plan-action (id 1) (plan-id ?planid) (goal-id ?goal-id)
              (action-name spawn-wp)
        ...
        (plan-action (id 6) (plan-id ?planid) (goal-id ?goal-id)
              ...
              (skiller (remote-skiller ?robot))
              (param-names r m side wp basecol)
              (param-values ?robot ?bs INPUT ?fwp ?some-base-color))
        (plan-action (id 7) (plan-id ?planid) (goal-id ?goal-id)
              (action-name wp-get)
              (skiller (remote-skiller ?robot))
              (param-names r wp m side)
...
(defrule goal-expander-produce-c1-mount-ring1
 ?p <- (goal (mode DISPATCHED) (id ?parent) (class PRODUCE-C1) (params order ?order bs-color ?base-color ring1-color ?ring1-color cs-color ?cap-color wp ?wp bs ?bs cs ?cs ds ?ds rs ?rs))
 ?g <- (goal (id ?goal-id) (parent ?parent) (class MOUNT-RING) (mode SELECTED))
 ...
 (not(goal (params robot ?robot)))
 =>
      (bind ?planid (sym-cat PRODUCE-C1-MOUNT-RING-PLAN- (gensym*)))
      (assert
        (plan (id ?planid) (goal-id ?goal-id))
        (plan-action (id 1) (plan-id ?planid) (goal-id ?goal-id)
              (action-name go-wait)
        ...
        (plan-action (id 5) (plan-id ?planid) (goal-id ?goal-id)
            ...
            (skiller (remote-skiller ?robot))
            (param-values ?robot ?rs ?wp ONE ?ring1-color RING_NONE RING_NONE RING_NONE ?bases-needed))
        (plan-action (id 6) (plan-id ?planid) (goal-id ?goal-id)
            (action-name go-wait)
            (skiller (remote-skiller ?robot))
            (param-names r from from-side to)
        ...
        (plan-action (id 13) (plan-id ?planid) (goal-id ?goal-id)
              (action-name location-unlock) (param-values ?rs OUTPUT))
        (plan-action (id 14) (plan-id ?planid) (goal-id ?goal-id)
              (action-name go-wait)
              (skiller (remote-skiller ?robot))
              (param-names r from from-side to)
              (param-values ?robot (wait-pos ?rs OUTPUT) WAIT (wait-pos ?cs INPUT)))
	  )
        (modify ?g (mode EXPANDED)(params robot ?robot))
)
