...
(defrule goal-expander-fill-cs
  ...
  (assert
    ...
    (plan-action (id 11) (plan-id ?plan-id) (goal-id ?goal-id)
      ...
      (skiller (remote-skiller ?robot))
      (param-names r from from-side to)
      (param-values ?robot ?cap-station OUTPUT (wait-pos ?cap-station OUTPUT))
    )  
  )
  (modify ?g (mode EXPANDED))
)
...
(defrule goal-expander-get-base
  ...
  (assert
    ...
    (plan-action (id 7) (plan-id ?plan-id) (goal-id ?goal-id)
      ...
      (skiller (remote-skiller ?robot))
      (param-names r from from-side to to-side)
      (param-values ?robot ?base-station ?bs-side ?cap-station INPUT)
    )
  )
  (modify ?g (mode EXPANDED))        
)
...
(defrule goal-expander-mount-cap
  ...
  (assert
    ...
    ; todo: where can the robot go to not block the station?
    (plan-action (id 7) (plan-id ?plan-id) (goal-id ?goal-id)
      (action-name go-wait)
      (skiller (remote-skiller ?robot))
      (param-names r from from-side to to-side)
      (param-values ?robot ?cap-station INPUT ?cap-station (wait-pos ?cap-station INPUT))
    )
  )
  (modify ?g (mode EXPANDED))
)
...
(defrule goal-expander-deliver-c0
  ...
  (assert
    ...
    (plan-action (id 7) (plan-id ?plan-id) (goal-id ?goal-id)
      ...
      (skiller (remote-skiller ?robot))
      (param-names ord wp m basecol capcol)
      (param-values ?order ?wp ?ds ?base-color ?cap-color)
    )
    (plan-action (id 8) (plan-id ?plan-id) (goal-id ?goal-id)
      (action-name go-wait)
      (skiller (remote-skiller ?robot))
      (param-names r from from-side to to-side)
      (param-values ?robot ?ds INPUT ?ds (wait-pos ?ds INPUT))
    )
  )
  (modify ?g (mode EXPANDED))
)
