...
(defrule goal-expander-buffer-rs
 ?p <- (goal (mode DISPATCHED) (id ?parent))
 ?g <- (goal (id ?goal-id) (parent ?parent) (class BUFFER-RS) (mode SELECTED) (params bs ?bs rs ?rs))
 (wm-fact (key domain fact entered-field args? r ?robot))
 (wm-fact (key domain fact at args? r ?robot m ?curr-location side ?curr-side))
 (wm-fact (key domain fact can-hold args? r ?robot))
 (not(goal (params robot ?robot $?rest-params)))
 =>
      ...
      (assert
        ...
        (modify ?g (mode EXPANDED)(params robot ?robot bs ?bs rs ?rs))
)
...
(defrule goal-expander-mount-ring1
 ...
 (not(goal (parent ?parent) (class BUFFER-RS) (params robot ?some-robot bs ?some-bs rs ?rs)))
 (not(goal (parent ?parent) (class BUFFER-RS) (params bs ?some-bs rs ?rs)))
