 ...
 =>
      ...
      (assert
        ...
        (plan-action (id 7) (plan-id ?planid) (goal-id ?goal-id)
              ...
              (skiller (remote-skiller ?robot))
              (param-values ?mps ?cc ?cs-color))
        (plan-action (id 8) (plan-id ?planid) (goal-id ?goal-id)
             (action-name location-unlock) (param-values ?mps INPUT))
        ...
        (plan-action (id 15) (plan-id ?planid) (goal-id ?goal-id)
              ...
              (skiller (remote-skiller ?robot))
              (param-names r from from-side to to-side )
              (param-values ?robot ?mps OUTPUT ?bs INPUT))
        ...
        (plan-action (id 17) (plan-id ?planid) (goal-id ?goal-id)
              ...
              (param-values ?robot ?bs INPUT ?wp ?bs-color))
        ...
        (plan-action (id 23) (plan-id ?planid) (goal-id ?goal-id)
              (action-name prepare-cs)
              (skiller (remote-skiller ?robot))
              (param-values ?mps MOUNT_CAP))
        (plan-action (id 24) (plan-id ?planid) (goal-id ?goal-id)
              (action-name cs-mount-cap)
              (skiller (remote-skiller ?robot))
              (param-values ?mps ?wp ?cs-color))
        (plan-action (id 25) (plan-id ?planid) (goal-id ?goal-id)
              (action-name move)
              (skiller (remote-skiller ?robot))
              (param-names r from from-side to to-side)
              (param-values ?robot ?mps INPUT ?mps OUTPUT))
        (plan-action (id 26) (plan-id ?planid) (goal-id ?goal-id)
              (action-name wp-get)
              (skiller (remote-skiller ?robot))
              (param-names r wp m side)
              (param-values ?robot ?wp ?mps OUTPUT))
        (plan-action (id 27) (plan-id ?planid) (goal-id ?goal-id)
              (action-name location-unlock) (param-values ?mps INPUT))
        (plan-action (id 28) (plan-id ?planid) (goal-id ?goal-id)
              (action-name location-lock)
              (param-values ?ds INPUT))
        (plan-action (id 29) (plan-id ?planid) (goal-id ?goal-id)
              (action-name move)
              (skiller (remote-skiller ?robot))
              (param-names r from from-side to to-side)
              (param-values ?robot ?mps OUTPUT ?ds INPUT))
        (plan-action (id 30) (plan-id ?planid) (goal-id ?goal-id)
              (action-name wp-put)
              (skiller (remote-skiller ?robot))
              (param-names r wp m)
              (param-values ?robot ?wp ?ds))
        (plan-action (id 31) (plan-id ?planid) (goal-id ?goal-id)
              (action-name prepare-ds)
              (skiller (remote-skiller ?robot))
              (param-values ?ds ?order))
        (plan-action (id 32) (plan-id ?planid) (goal-id ?goal-id)
              (action-name fulfill-order-c0)
              (skiller (remote-skiller ?robot))
              (param-values ?order ?wp ?ds INPUT ?bs-color ?cs-color))
        (plan-action (id 33) (plan-id ?planid) (goal-id ?goal-id)
