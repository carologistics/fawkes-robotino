...
(deffunction g1-goal-production-assert-c2
  ...
  (bind ?goal-tree-4
    (goal-tree-assert-central-run-all-sequence (sym-cat PRODUCT- ?ord -PT) 1 1
      ...
      (goal-production-g1-c1-mount-ring1 ?ord ?rs2 ?rng-clr2 ?r-req2 ?wp PRIMARY_TASK 12)
      ...
      (goal-production-g1-c1-deliver ?ord ?ds ?ds-gate ?base-clr ?cap-clr ?rng-clr1 ?rng-clr2  ?wp PRIMARY_TASK 16)   ; Need to change this to accomodate ring 2 and ring 3
    )
  )
...
;;;;;;;;;;;;;;   GROUP-1 COMPLEXITY-3 PAYMENT PARALLEL    ;;;;;;;;;;;;;;;;;;;;
...
(deffunction g1-goal-production-assert-c3
  ...
  (bind ?goal-tree-4
    (if (neq ?r-req3 TWO) then
      (if (eq ?r-req3 ONE) then
        (create$
          (goal-tree-assert-central-run-parallel (sym-cat PRODUCT- ?ord -ST4) 2 1
            (goal-production-g1-c1-make-payment-cs ?ord ?cs ?rs3 SECONDARY_TASK 5)
          )
        )
      )
    else
      (create$
        (goal-tree-assert-central-run-parallel (sym-cat PRODUCT- ?ord -ST4) 2 1
          (goal-production-g1-c1-make-payment-cs ?ord ?cs ?rs3 SECONDARY_TASK 5)
          (goal-production-g1-c1-make-payment-bs ?ord ?bs ?rs3 SECONDARY_TASK 6)
        )
      )
    )
  )
  ...
  (bind ?goal-tree-4
    (goal-tree-assert-central-run-all-sequence (sym-cat PRODUCT- ?ord -PT) 1 1
      ...
      (goal-production-g1-c1-prepare-rs ?ord ?rs2 ?rng-clr2 ?r-req2 PRIMARY_TASK 11)
      (goal-production-g1-c1-mount-ring1 ?ord ?rs2 ?rng-clr2 ?r-req2 ?wp PRIMARY_TASK 12)
      (goal-production-g1-c1-transport-wp ?ord ?rs2 OUTPUT ?rs3 INPUT ?wp PRIMARY_TASK 13)
      (goal-production-g1-c1-prepare-rs ?ord ?rs3 ?rng-clr3 ?r-req3 PRIMARY_TASK 14)
      (goal-production-g1-c1-mount-ring1 ?ord ?rs3 ?rng-clr3 ?r-req3 ?wp PRIMARY_TASK 15)
      (goal-production-g1-c1-transport-wp ?ord ?rs3 OUTPUT ?cs INPUT ?wp PRIMARY_TASK 16)
      (goal-production-g1-c1-cap-mount ?ord ?cs ?cap-clr ?wp PRIMARY_TASK 17)
      (goal-production-g1-c1-transport-wp ?ord ?cs OUTPUT ?ds INPUT ?wp PRIMARY_TASK 18)
      (goal-production-g1-c1-deliver ?ord ?ds ?ds-gate ?base-clr ?cap-clr ?rng-clr1 ?rng-clr2 ?rng-clr3 ?wp PRIMARY_TASK 19)   ;need to change this to accomodate the 3 rings
    )
  )
...
(defrule g1-goal-production-create-from-order-complexity-C3
  ...
  (wm-fact (key domain fact order-complexity args?  ord ?ord com C3)) 
	(wm-fact (key domain fact order-ring1-color args? ord ?ord col ?rng-clr1))
  (wm-fact (key domain fact order-ring2-color args? ord ?ord col ?rng-clr2))
  (wm-fact (key domain fact order-ring2-color args? ord ?ord col ?rng-clr3))  
	;(wm-fact (key domain fact order-complexity args?  ord  ?ord comp ?ord-cmplx))
	(wm-fact (key domain fact order-base-color args? ord ?ord  col ?base-clr)) 
	(wm-fact (key domain fact order-cap-color args? ord ?ord col ?cap-clr)) 
	(wm-fact (key domain fact order-gate args? ord ?ord gate ?ds-gate)) 
  ...
  (wm-fact (key refbox team-color) (value ?team-color)) 
	...
	=>
	(bind ?ord-comp C3)     ; just for information
  (bind ?rnd-id (sym-cat ?ord - (gensym*) ))
  (bind ?wp (sym-cat WP - ?ord))
  ; (assert (domain-object (name ?wp) (type workpiece)))
  ...
  (if (eq ?cap-clr CAP_BLACK)
	...
	(bind ?goal-tree (g1-goal-production-assert-c2 ?base-clr ?rs1 ?rng-clr1 ?rs2 ?rng-clr2 ?rs3 ?rng-clr3 ?cs ?cap-clr C-DS ?ds-gate ?ord ?wp))
)