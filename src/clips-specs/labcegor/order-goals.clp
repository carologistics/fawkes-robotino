...
(defrule assign-c3-order
    ...
    (bind ?goal-id-payring1 (sym-cat GOAL-PAY-FIRST-RING- ?ord ))
    (bind ?goal-id-payring2 (sym-cat GOAL-PAY-SECOND-RING- ?ord ))
    (bind ?goal-id-payring3 (sym-cat GOAL-PAY-THIRD-RING- ?ord ))
    (bind ?goal-id-bs (sym-cat GOAL-GET-BS- ?ord ))
    (bind ?goal-id-parallel1 (sym-cat GOAL-PARALLEL-TO-RS1-AND-PREPARE-RS2- ?ord))
    (bind ?gial-id-parallel2 (sym-cat GOAL-PARALLEL-TO-RS2-AND-PREPARE-RS3- ?ord))
    ...
    ; Goal Pay Third Ring
    (assert (goal (class GOAL-PAY-THIRD-RING)
                (id ?goal-id-payring3)
                (type ACHIEVE)
                (sub-type SIMPLE)
                (parent ?goal-id-parallel2)
...
; expansion rule to pay for third ring (nothing)
(defrule goal-expander-goal-pay-third-ring-cc0
    ?g <- (goal (id ?goal-id) (class GOAL-PAY-THIRD-RING) (mode SELECTED) (parent ?parent))
    ?m <- (goal-meta (goal-id ?goal-id) (order-id ?ord))

    (domain-fact (name order-ring3-color) (param-values ?ord ?ring3col))
    (domain-fact (name rs-ring-spec) (param-values ?rs ?ring3col ZERO))
...
; expansion rule to pay for third ring (one payment) (ATTENTION: THIS SELECTS THE SAME CAP STATION AS WE NEED LATER FOR OUR CAP)
(defrule goal-expander-goal-pay-third-ring-cc1
    ?g <- (goal (id ?goal-id) (class GOAL-PAY-third-RING) (mode SELECTED) (parent ?parent))
    ?m <- (goal-meta (goal-id ?goal-id) (order-id ?ord))
    ?rw <- (domain-fact (name robot-waiting) (param-values ?robot))
    ...
    (domain-fact (name order-ring3-color) (param-values ?ord ?ring3col))
    (domain-fact (name order-cap-color) (param-values ?ord ?capcol))
    (domain-fact (name rs-ring-spec) (param-values ?rs ?ring3col ONE))
    (domain-fact (name rs-filled-with) (param-values ?rs ?rs-before))
    (domain-fact (name rs-inc) (param-values ?rs-before ?rs-after))
    ...
    =>
    (plan-assert-sequential PAY-THIRD-RING-CC1 ?goal-id ?robot
        (plan-assert-action move ?robot ?fl1 ?fs1 ?cs INPUT)
        (plan-assert-action wp-get-shelf ?robot ?wp ?cs ?spot)
        (plan-assert-action refill-shelf ?cs ?spot (gensym*) ?capcol)
        (plan-assert-action move ?robot ?cs INPUT ?rs INPUT)
    ...
    (printout t "Robot " ?robot " was assigned PAY-THIRD-RING-" ?ord crlf)
    (retract ?rw)  ;We must insert this again when done executing!
)
...
; expansion rule to pay for third ring (two payments) (ATTENTION: THIS SELECTS THE SAME CAP STATION AS WE NEED LATER FOR OUR CAP)
(defrule goal-expander-goal-pay-third-ring-cc2
    ?g <- (goal (id ?goal-id) (class GOAL-PAY-THIRD-RING) (mode SELECTED) (parent ?parent))
    ?m <- (goal-meta (goal-id ?goal-id) (order-id ?ord))
    ?rw <- (domain-fact (name robot-waiting) (param-values ?robot))
    ...
    (domain-fact (name order-ring3-color) (param-values ?ord ?ring3col))
    (domain-fact (name order-cap-color) (param-values ?ord ?capcol))
    (domain-fact (name rs-ring-spec) (param-values ?rs ?ring3col TWO))
    (domain-fact (name rs-filled-with) (param-values ?rs ?rs-before))
    (domain-fact (name rs-inc) (param-values ?rs-before ?rs-after1))
    (domain-fact (name rs-inc) (param-values ?rs-after1 ?rs-after2))
    ...
    =>
    (plan-assert-sequential PAY-THIRD-RING-CC2 ?goal-id ?robot
        (plan-assert-action move ?robot ?fl1 ?fs1 ?cs1 INPUT)
        (plan-assert-action wp-get-shelf ?robot ?wp1 ?cs1 ?spot1)
        (plan-assert-action refill-shelf ?cs1 ?spot1 (gensym*) ?capcol)
        (plan-assert-action move ?robot ?cs1 INPUT ?rs INPUT)
