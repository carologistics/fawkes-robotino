...
(define (domain rcll-production)
	...
	(:action cs-mount-cap
		...
		:precondition (and (mps-type ?m CS) (mps-state ?m PROCESSING) (locked ?m)
	...
	(:action rs-mount-ring1
		...
		:precondition (and (mps-type ?m RS) (mps-state ?m PROCESSING) (locked ?m)
...
;  )
	...
	(:action wp-put
		...
		:precondition (and (at ?r ?m INPUT) (mps-state ?m PREPARED) (locked ?m)
