...
(defrule goal-expander-prefill-cap-station
   ...
   (assert
        ...
        (plan-action (id 6) (plan-id FILL-CAP-PLAN) (goal-id FILL-CAP)
                                    (action-name wp-put)
                                    (param-names r wp m)
                                    (param-values ?robot ?cc ?mps))
        (plan-action (id 7) (plan-id FILL-CAP-PLAN) (goal-id FILL-CAP)
                                    (action-name cs-retrieve-cap)
                                    (param-names m cc capcol)
                                    (param-values ?mps ?cc ?cap-color))
        (plan-action (id 8) (plan-id FILL-CAP-PLAN) (goal-id FILL-CAP)
                                    (action-name unlock)
                                    (param-values ?mps))
        (plan-action (id 9) (plan-id FILL-CAP-PLAN) (goal-id FILL-CAP)
...
(defrule goal-remove-empty-base-from-cs
 ...
 (assert
  ...
  (plan-action (id 3) (plan-id CLEAR-CS-PLAN) (goal-id CLEAR-CS)
        (action-name lock) (param-values ?mps))
  (plan-action (id 4) (plan-id CLEAR-CS-PLAN) (goal-id CLEAR-CS)
        (action-name wp-get)
        (param-names r wp m side)
        (param-values ?robot ?wp ?mps OUTPUT))
  (plan-action (id 5) (plan-id CLEAR-CS-PLAN) (goal-id CLEAR-CS)
        (action-name unlock) (param-values ?mps))
  (plan-action (id 6) (plan-id CLEAR-CS-PLAN) (goal-id CLEAR-CS)
                              (action-name location-unlock)
                              (param-values ?mps OUTPUT))
 )
 (modify ?g (mode EXPANDED))
 ...
 (wm-fact (key domain fact at args? r ?robot m ?curr-location side ?curr-side))
  ...
  (assert
    ...
    (plan-action (id 2) (plan-id FILL-RS-PLAN) (goal-id FILL-RS)
                                    ...
                                    (param-names r from from-side to to-side )
                                    (param-values ?robot ?curr-location ?curr-side ?mps INPUT))
    (plan-action (id 3) (plan-id FILL-RS-PLAN) (goal-id FILL-RS)
        (action-name lock) (param-values ?mps))
    (plan-action (id 4) (plan-id FILL-RS-PLAN) (goal-id FILL-RS)
          (action-name wp-put-slide-cc)
          (param-names r wp m rs-before rs-after)
          (param-values ?robot ?wp ?mps ?rs-before ?rs-after))
    (plan-action (id 5) (plan-id FILL-RS-PLAN) (goal-id FILL-RS)
        (action-name unlock) (param-values ?mps))
    (plan-action (id 6) (plan-id FILL-RS-PLAN) (goal-id FILL-RS)
                                    (action-name location-unlock)
                                    (param-values ?mps INPUT))
  )
  (modify ?g (mode EXPANDED))
  ...
  then
   ...
   (assert
    ...
    (plan-action (id 8) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name location-unlock)
          (param-values ?bs ?bs-side))
    (plan-action (id 9) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name location-lock)
          (param-values ?mps INPUT))
    (plan-action (id 10) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name move)
          (param-names r from from-side to)
          (param-values ?robot ?bs ?bs-side ?mps))
    (plan-action (id 11) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name lock) (param-values ?mps))
    (plan-action (id 12) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name prepare-cs)
          (param-names m op)
          (param-values ?mps MOUNT_CAP))
    (plan-action (id 13) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name wp-put)
          (param-names r wp m)
          (param-values ?robot ?spawned-wp ?mps))
     (plan-action (id 14) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name cs-mount-cap)
          (param-names m wp capcol)
          (param-values ?mps ?spawned-wp ?cap-color ))
    (plan-action (id 15) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name unlock) (param-values ?mps))
    (plan-action (id 16) (plan-id PRODUCE-C0-PLAN) (goal-id PRODUCE-C0)
          (action-name location-unlock)
          (param-values ?mps INPUT))
   )
  (modify ?g (mode EXPANDED))
   ...
   (modify ?g (mode FINISHED) (outcome FAILED)
     ...
     (assert
      ...
      (plan-action (id 8) (plan-id MOUNT-FIRST-RING-PLAN) (goal-id MOUNT-FIRST-RING)
            (action-name lock) (param-values ?mps))
      (plan-action (id 9) (plan-id MOUNT-FIRST-RING-PLAN) (goal-id MOUNT-FIRST-RING)
            (action-name prepare-rs)
            (param-names m rc rs-before rs-after rs-req)
            (param-values ?mps ?ring-color ?rs-before ?rs-after ?rs-req))
      (plan-action (id 10) (plan-id MOUNT-FIRST-RING-PLAN) (goal-id MOUNT-FIRST-RING)
            (action-name wp-put)
            (param-names r wp m)
            (param-values ?robot ?spawned-wp ?mps))
       (plan-action (id 11) (plan-id MOUNT-FIRST-RING-PLAN) (goal-id MOUNT-FIRST-RING)
      ...
      (plan-action (id 12) (plan-id MOUNT-FIRST-RING-PLAN) (goal-id MOUNT-FIRST-RING)
            (action-name unlock) (param-values ?mps))
     )
    (modify ?g (mode EXPANDED))
  )
...
(defrule goal-deliver
 ...
 (assert
  ...
  (plan-action (id 6) (plan-id DELIVER-PLAN) (goal-id DELIVER)
        (action-name location-unlock)
        (param-values ?mps OUTPUT))
  (plan-action (id 7) (plan-id DELIVER-PLAN) (goal-id DELIVER)
        (action-name location-lock)
  ...
  (plan-action (id 10) (plan-id DELIVER-PLAN) (goal-id DELIVER)
        (action-name prepare-ds)
        (param-names m gate)
        (param-values ?ds ?gate))
  (plan-action (id 11) (plan-id DELIVER-PLAN) (goal-id DELIVER)
        (action-name wp-put)
        (param-names r wp m)
        (param-values ?robot ?wp ?ds))
  (plan-action (id 12) (plan-id DELIVER-PLAN) (goal-id DELIVER)
        (action-name fulfill-order-c0)
        (param-names ord wp m g basecol capcol)
        (param-values ?order ?wp ?ds ?gate ?base-color  ?cap-color))
  (plan-action (id 13) (plan-id DELIVER-PLAN) (goal-id DELIVER)
        (action-name unlock) (param-values ?ds))
  (plan-action (id 14) (plan-id DELIVER-PLAN) (goal-id DELIVER)
        (action-name location-unlock)
        (param-values ?ds INPUT))
 )
 (modify ?g (mode EXPANDED))
