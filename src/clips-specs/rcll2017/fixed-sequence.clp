...
(defrule goal-produce-garbage-c3
 ?p <- (goal (mode EXPANDED) (id ?parent))
 ?g <- (goal (mode SELECTED) (parent ?parent) (id GARBAGE)
                             (params robot ?robot
                                        bs ?bs
                                        rs1 ?mps-rs1
                                        rs2 ?mps-rs2
 ...
 (if (eq ?spawned-wp NA)
  then
   ...
   (modify ?g (mode FINISHED) (outcome FAILED)
     ...
     (assert
      (plan (id GARBAGE-PLAN) (goal-id GARBAGE))
      (plan-action (id 1) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name move)
            (param-names r from from-side to to-side )
            (param-values ?robot ?curr-location ?curr-side ?bs OUTPUT))
      (plan-action (id 2) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name prepare-bs)
            (param-names m side bc)
            (param-values ?bs ?bs-side ?base-color))
      (plan-action (id 3) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name bs-dispense)
            (param-names r m side wp basecol)
            (param-values ?robot ?bs ?bs-side ?spawned-wp ?base-color))
      (plan-action (id 4) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name wp-get)
            (param-names r wp m side)
            (param-values ?robot ?spawned-wp ?bs ?bs-side))
      (plan-action (id 5) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name move)
            (param-names r from from-side to to-side )
            (param-values ?robot ?bs OUTPUT ?mps-rs1 INPUT))
      (plan-action (id 6) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name wp-put)
            (param-names r wp m)
            (param-values ?robot ?spawned-wp ?mps-rs1))
      (plan-action (id 7) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name prepare-rs)
            (param-names m rc rs-before rs-after r-req)
            (param-values ?mps-rs1 ?ring1-color ZERO ZERO ZERO))
       (plan-action (id 8) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name rs-mount-ring1)
            (param-names m wp col rs-before rs-after r-req)
            (param-values ?mps-rs1 ?spawned-wp ?ring1-color ZERO ZERO ZERO))
      (plan-action (id 9) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name move)
            (param-names r from from-side to to-side )
            (param-values ?robot ?mps-rs1 INPUT ?mps-rs1 OUTPUT))
      (plan-action (id 10) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name wp-get)
            (param-names r wp m side)
            (param-values ?robot ?spawned-wp ?mps-rs1 OUTPUT))
      (plan-action (id 11) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name move)
            (param-names r from from-side to to-side )
            (param-values ?robot ?mps-rs1 OUTPUT ?mps-rs2 INPUT))
      (plan-action (id 12) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name wp-put)
            (param-names r wp m)
            (param-values ?robot ?spawned-wp ?mps-rs2))
      (plan-action (id 13) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name prepare-rs)
            (param-names m rc rs-before rs-after r-req)
            (param-values ?mps-rs2 ?ring2-color ZERO ZERO ZERO))
       (plan-action (id 14) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name rs-mount-ring2)
            (param-names m wp col rs-before rs-after r-req)
            (param-values ?mps-rs2 ?spawned-wp ?ring1-color ?ring2-color ZERO ZERO ZERO))
      (plan-action (id 15) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name move)
            (param-names r from from-side to to-side )
            (param-values ?robot ?mps-rs2 INPUT ?mps-rs2 OUTPUT))
      (plan-action (id 16) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name wp-get)
            (param-names r wp m side)
            (param-values ?robot ?spawned-wp ?mps-rs2 OUTPUT))
      (plan-action (id 17) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name move)
            (param-names r from from-side to to-side )
            (param-values ?robot ?mps-rs2 OUTPUT ?mps-rs1 INPUT))
      (plan-action (id 18) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name wp-put)
            (param-names r wp m)
            (param-values ?robot ?spawned-wp ?mps-rs1))
      (plan-action (id 19) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name prepare-rs)
            (param-names m rc rs-before rs-after r-req)
            (param-values ?mps-rs1 ?ring1-color ZERO ZERO ZERO))
       (plan-action (id 20) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name rs-mount-ring2)
            (param-names m wp col rs-before rs-after r-req)
            (param-values ?mps-rs1 ?spawned-wp ?ring1-color ?ring2-color ?ring1-color ZERO ZERO ZERO))
      (plan-action (id 21) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name move)
            (param-names r from from-side to to-side )
            (param-values ?robot ?mps-rs1 INPUT ?mps-rs1 OUTPUT))
      (plan-action (id 22) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
            (action-name wp-get)
            (param-names r wp m side)
            (param-values ?robot ?spawned-wp ?mps-rs1 OUTPUT))
    (plan-action (id 23) (plan-id GARBAGE-PLAN) (goal-id GARBAGE)
          (action-name wp-discard)
          (param-names r cc )
          (param-values ?robot ?spawned-wp))
     )
    (modify ?g (mode EXPANDED))
  )
)
