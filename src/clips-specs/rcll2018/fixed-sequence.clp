...
(defrule goal-expander-prefill-cap-station
   ...
   it can directly put the cap on a product."
    ...
    =>
   (assert
        (plan (diag-wm-store TRUE) (id FILL-CAP-PLAN) (goal-id ?goal-id))
        ...
        (plan-action (id 4) (plan-id FILL-CAP-PLAN) (goal-id ?goal-id)
                                    ...
                                    (param-names r cc m side spot hold)
                                    (param-values ?robot ?cc ?mps INPUT ?shelf-spot DUMMY-WP))
        (plan-action (id 5) (plan-id FILL-CAP-PLAN) (goal-id ?goal-id)
                                    (action-name lock)
                                    (param-values ?mps))
        (plan-action (id 6) (plan-id FILL-CAP-PLAN) (goal-id ?goal-id)
                                    (action-name wp-put)
                                    (param-names r wp m side there)
                                    (param-values ?robot ?cc ?mps INPUT DUMMY-WP))
    ...
    (modify ?g (mode EXPANDED) (committed-to FILL-CAP-PLAN))
)
...
(defrule goal-remove-workpiece-from-mps
 ...
 (assert
  ...
  (plan-action (id 5) (plan-id CLEAR-MPS-PLAN) (goal-id ?goal-id)
        (action-name wp-get)
        (param-names r wp m side hold)
        (param-values ?robot ?wp ?mps ?side DUMMY-WP))
  (plan-action (id 6) (plan-id CLEAR-MPS-PLAN) (goal-id ?goal-id)
        (action-name unlock) (param-values ?mps))
  (plan-action (id 7) (plan-id CLEAR-MPS-PLAN) (goal-id ?goal-id)
                              (action-name location-unlock)
                              (param-values ?mps ?side))
  (plan-action (id 8) (plan-id CLEAR-MPS-PLAN) (goal-id ?goal-id)
        (action-name go-wait)
        (param-names r from from-side to)
...
(defrule goal-expander-fill-rs
 ...
 (wm-fact (key domain fact at args? r ?robot m ?curr-location side ?curr-side))
  =>
  (assert
    (plan (diag-wm-store TRUE) (id FILL-RS-PLAN) (goal-id ?goal-id))
    ...
    (plan-action (id 4) (plan-id FILL-RS-PLAN) (goal-id ?goal-id)
          ...
          (param-names r wp m side rs-before rs-after)
          (param-values ?robot ?wp ?mps INPUT ?rs-before ?rs-after))
...
(defrule goal-expander-get-base-to-fill-rs
 ...
 =>
 (assert
  (plan (diag-wm-store TRUE) (id GET-BASE-TO-FILL-RS-PLAN) (goal-id ?goal-id))
  ...
  (plan-action (id 7) (plan-id  GET-BASE-TO-FILL-RS-PLAN) (goal-id ?goal-id)
        (action-name wp-get)
        (param-names r wp m side hold)
        (param-values ?robot ?spawned-wp ?bs ?bs-side DUMMY-WP))
...
(defrule goal-expander-get-shelf-to-fill-rs
 ...
 (wm-fact (key domain fact at args? r ?robot m ?curr-location side ?curr-side))
  =>
 (assert
    (plan (diag-wm-store TRUE) (id GET-SHELF-TO-FILL-RS-PLAN) (goal-id ?goal-id))
    ...
    (plan-action (id 4) (plan-id  GET-SHELF-TO-FILL-RS-PLAN) (goal-id ?goal-id)
          (action-name wp-get-shelf)
          (param-names r cc m side spot hold)
          (param-values ?robot ?wp ?cs INPUT ?spot DUMMY-WP))
...
(defrule goal-produce-c0
 ...
 (assert
  ...
  (plan-action (id 14) (plan-id PRODUCE-C0-PLAN) (goal-id ?goal-id)
        (action-name wp-put)
        (param-names r wp m side there)
        (param-values ?robot ?spawned-wp ?mps INPUT DUMMY-WP))
  ...
  (plan-action (id 17) (plan-id PRODUCE-C0-PLAN) (goal-id ?goal-id)
        (action-name unlock) (param-values ?mps))
  (plan-action (id 18) (plan-id PRODUCE-C0-PLAN) (goal-id ?goal-id)
        (action-name location-unlock)
        (param-values ?mps INPUT))
  (plan-action (id 19) (plan-id PRODUCE-C0-PLAN) (goal-id ?goal-id)
        (action-name go-wait)
        (param-names r from from-side to)
        (param-values ?robot ?mps INPUT (wait-pos ?mps INPUT)))
 )
...
(defrule goal-mount-first-ring
 ...
 =>
 (assert
  (plan (diag-wm-store TRUE) (id MOUNT-FIRST-RING-PLAN) (goal-id ?goal-id))
  ...
  (plan-action (id 17) (plan-id MOUNT-FIRST-RING-PLAN) (goal-id ?goal-id)
        (action-name unlock) (param-values ?mps))
  (plan-action (id 18) (plan-id MOUNT-FIRST-RING-PLAN) (goal-id ?goal-id)
        (action-name location-unlock)
        (param-values ?mps INPUT))
  (plan-action (id 19) (plan-id MOUNT-FIRST-RING-PLAN) (goal-id ?goal-id)
        (action-name go-wait)
        (param-names r from from-side to)
        (param-values ?robot ?mps INPUT (wait-pos ?mps INPUT)))
 )
 (modify ?g (mode EXPANDED) (committed-to MOUNT-FIRST-RING-PLAN))
)
...
(defrule goal-mount-next-ring
 ...
 =>
    ...
    (assert
      (plan (diag-wm-store TRUE) (id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id))
      ...
      (plan-action (id 5) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name wp-get)
            (param-names r wp m side hold)
            (param-values ?robot ?wp ?prev-rs ?prev-rs-side DUMMY-WP))
      (plan-action (id 6) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name unlock)
            (param-values ?prev-rs))
      (plan-action (id 7) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name location-unlock)
            (param-values ?prev-rs ?prev-rs-side))
      (plan-action (id 8) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name go-wait)
            (param-names r from from-side to)
            (param-values ?robot ?prev-rs ?prev-rs-side (wait-pos ?rs INPUT)))
      (plan-action (id 9) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name location-lock)
      ...
      (plan-action (id 11) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name lock)
            (param-values ?rs))
      (plan-action (id 12) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name wp-put)
            (param-names r wp m side there)
            (param-values ?robot ?wp ?rs INPUT DUMMY-WP))
      (plan-action (id 13) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name prepare-rs)
            (param-names m rc rs-before rs-after r-req)
            (param-values ?rs ?curr-ring-color ?rs-before ?rs-after ?rs-req))
       (plan-action (id 14) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name ?mount-ring-action-name)
            (param-names ?mount-ring-param-names)
            (param-values ?mount-ring-param-values))
      (plan-action (id 15) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name unlock)
            (param-values ?rs))
      (plan-action (id 16) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name location-unlock)
            (param-values ?rs INPUT))
      (plan-action (id 17) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name go-wait)
            (param-names r from from-side to)
 ...
 =>
   (assert
    ...
    (plan-action (id 5) (plan-id PRODUCE-CX-PLAN) (goal-id ?goal-id)
          (action-name wp-get)
          (param-names r wp m side hold)
          (param-values ?robot ?wp ?rs OUTPUT DUMMY-WP))
    ...
    (plan-action (id 12) (plan-id PRODUCE-CX-PLAN) (goal-id ?goal-id)
          (action-name wp-put)
          (param-names r wp m side there)
          (param-values ?robot ?wp ?mps INPUT DUMMY-WP))
    ...
    (plan-action (id 15) (plan-id PRODUCE-CX-PLAN) (goal-id ?goal-id)
          (action-name unlock)
          (param-values ?mps))
    (plan-action (id 16) (plan-id PRODUCE-CX-PLAN) (goal-id ?goal-id)
          (action-name location-unlock)
          (param-values ?mps INPUT))
    (plan-action (id 17) (plan-id PRODUCE-CX-PLAN) (goal-id ?goal-id)
          (action-name go-wait)
          (param-names r from from-side to)
          (param-values ?robot ?mps INPUT (wait-pos ?mps INPUT)))
   )
...
(defrule goal-deliver
 ...
 (assert
  ...
  (plan-action (id 5) (plan-id DELIVER-PLAN) (goal-id ?goal-id)
        (action-name wp-get)
        (param-names r wp m side hold)
        (param-values ?robot ?wp ?mps OUTPUT DUMMY-WP))
  ...
  (plan-action (id 12) (plan-id DELIVER-PLAN) (goal-id ?goal-id)
        (action-name wp-put)
        (param-names r wp m side there)
        (param-values ?robot ?wp ?ds INPUT DUMMY-WP))
 ...
 (assert
   ...
   (plan-action (id 15) (plan-id DELIVER-PLAN) (goal-id ?goal-id)
        (action-name unlock) (param-values ?ds))
   (plan-action (id 16) (plan-id DELIVER-PLAN) (goal-id ?goal-id)
        (action-name location-unlock)
        (param-values ?ds INPUT))
   (plan-action (id 17) (plan-id DELIVER-PLAN) (goal-id ?goal-id)
        (action-name go-wait)
        (param-names r from from-side to)
