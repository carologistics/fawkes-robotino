...
(defrule goal-expander-prefill-cap-station
   ...
   (assert
        ...
        (plan-action (id 4) (plan-id FILL-CAP-PLAN) (goal-id ?goal-id)
                                    ...
                                    (param-names r cc m spot hold)
                                    (param-values ?robot ?cc ?mps ?shelf-spot DUMMY-WP))
        (plan-action (id 5) (plan-id FILL-CAP-PLAN) (goal-id ?goal-id)
                                    (action-name wp-put)
                                    (param-names r wp m)
                                    (param-values ?robot ?cc ?mps DUMMY-WP))
...
(defrule goal-produce-c0
 ...
 (assert
  ...
  (plan-action (id 14) (plan-id PRODUCE-C0-PLAN) (goal-id ?goal-id)
        (action-name wp-put)
        (param-names r wp m there)
        (param-values ?robot ?spawned-wp ?mps DUMMY-WP))
