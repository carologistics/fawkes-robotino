...
(defrule goal-expander-discard-unneeded-base
 ...
 ?g <- (goal (id ?goal-id) (class DISCARD-UNKNOWN) (mode SELECTED)
  ...
  (assert
    ...
    (plan-action (id 2) (plan-id DISCARD-UNKNOWN-PLAN) (goal-id ?goal-id)
          (action-name gripper-close)
          (param-names r)
          (param-values ?robot))
  )
  (modify ?g (mode EXPANDED))
...
(defrule goal-mount-next-ring
 ...
 =>
    ...
    (if (eq ?prev-rs PARTIAL)
     then
        (bind ?prev-rs  ?curr-location)
        (bind ?prev-rs-side ?curr-side)
     else
        (assert
          (plan (id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id))
      ...
      (assert (plan-action (id 7) (plan-id MOUNT-NEXT-RING-PLAN) (goal-id ?goal-id)
            (action-name location-lock)
...
(defrule goal-produce-cx
 ...
 (wm-fact (key domain fact at args? r ?robot m ?curr-location side ?curr-side))
 =>
   (assert (plan (id PRODUCE-CX-PLAN) (goal-id ?goal-id)))
   (if (eq ?rs PARTIAL)
     then
      (bind ?rs  ?curr-location)
      (bind ?rs-side ?curr-side)
     else
      (bind ?rs-side OUTPUT)
      (assert
...
(plan-action (id 8) (plan-id PRODUCE-CX-PLAN) (goal-id ?goal-id)
          (action-name move)
          (param-names r from from-side to to-side )
          (param-values ?robot ?rs ?rs-side ?mps INPUT))
...
(defrule goal-deliver
 ...
 =>
  (assert (plan (id DELIVER-PLAN) (goal-id ?goal-id)))
   (if (eq ?mps PARTIAL)
     then
      (bind ?mps  ?curr-location)
      (bind ?mps-side  ?curr-side)
     else
      (bind ?mps-side OUTPUT)
      (assert
