...
" Spawn a C0 into the storage station. It has the same base color as the
  standing C0 order and the opposite cap.
"
  (declare (salience ?*SALIENCE-GOAL-FORMULATE*))
  ?g <- (goal (id ?maintain-id) (class WP-SPAWN-MAINTAIN) (mode SELECTED))
  (not (goal (class SPAWN-WP)))
...
"
  ...
  (if (any-factp ((?exclusive-complexities wm-fact))
    ...
    then
      ...
      (bind ?required-resources ?order PRODUCE-EXCLUSIVE-COMPLEXITY)
...
"
  ...
  (wm-fact (key domain fact order-ring1-color args? ord ?order col ?ring1-color))
  (wm-fact (key refbox order ?order quantity-requested) (value ?qr))
  (wm-fact (key domain fact quantity-delivered ?order ?team-color) (value ?qd&:(> ?qr ?qd)))
  ;Active Order CEs
