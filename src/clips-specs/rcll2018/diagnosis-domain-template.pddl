...
;Kind of a hack. actually it should model the removal of present workpieces
	...
	(:action prepare-cs
		...
		:effect (and (not (next-prepare-cs ?m ?op))
								 ...
								 (when (and (comp-state ?m IDLE) (not (mps-side-free ?m INPUT)) (cs-can-perform ?m ?op))
								 		(and (not (comp-state ?m IDLE)) (comp-state ?m READY-AT-OUTPUT)
	...
	(:action prepare-rs
		...
		:effect (and (not (next-prepare-rs ?m ?rc)) (last-prepare-rs ?m ?rc)
								 (when (and (comp-state ?m IDLE) (rs-filled-with ?m ?rs-before)
								 			 )
											 (and (not (comp-state ?m IDLE)) (comp-state ?m READY-AT-OUTPUT)
	...
	(:action rs-mount-ring1
		...
		:precondition (and (next-rs-mount-ring1 ?m) 
										(mps-type ?m RS) (comp-state ?m READY-AT-OUTPUT) (locked ?m)
...
(not (spot-free ?m ?spot) )
  ...
  (:action lock
    ...
    :precondition (and (comp-state communication CONNECTION-ESTABLISHED) (next-lock ?name) (not (locked ?name)))
    :effect (and (not (exog-possible))
  ...
  (:action one-time-lock
    :parameters (?name - object)
    :precondition (and (comp-state communication CONNECTION-ESTABLISHED) (next-one-time-lock ?name) (not (locked ?name)))
    :effect (and (not (exog-possible))
(not (next-one-time-lock ?name)) (last-one-time-lock ?name)
  ...
  (:action unlock
    :parameters (?name - object)
    :precondition (and (comp-state communication CONNECTION-ESTABLISHED) (next-unlock ?name) (locked ?name))
    :effect (and (not (exog-possible))
(not (next-unlock ?name)) (last-unlock ?name)
  ...
  (:action eventually-unlock
    :parameters (?name - object)
    :precondition (and (comp-state communication CONNECTION-ESTABLISHED) (next-eventually-unlock ?name) (locked ?name))
    :effect (and (not (exog-possible))
(not (next-eventually-unlock ?name)) (last-eventually-unlock ?name)
  ...
  (:action location-lock
    ...
    :precondition (and (comp-state communication CONNECTION-ESTABLISHED) (next-location-lock ?location ?side) 
...
(not (next-location-lock ?location ?side)) (last-location-lock ?location ?side)
  ...
  (:action location-unlock
    :parameters (?location - mps ?side - mps-side)
    :precondition (and (comp-state communication CONNECTION-ESTABLISHED) (next-location-unlock ?location ?side) 
						(location-locked ?location ?side)
				  )
    :effect (and (not (exog-possible))
