...
(defrule goal-class-create-pickup-wp-rs
    "When both RS are blocked, pick up a workpiece from a ring station."
    (wm-fact (key refbox team-color) (value ?team-color))
    (wm-fact (key domain fact self args? r ?robot))

    (wm-fact (key domain fact mps-type args? m ?rs t RS))
    (wm-fact (key domain fact mps-type args? m ?other-rs&~?rs t RS))

    (not (goal-class (class PICKUP-WP) (meta mps ?rs)))
    =>
    (assert
        (goal-class (class PICKUP-WP)
                    (id (sym-cat PICKUP-WP- ?rs))
                    (meta mps ?rs)
                    (type ACHIEVE)
                    (sub-type SIMPLE)
                    (param-names     rs  other-rs  wp        robot  order)
                    (param-constants ?rs ?other-rs nil       ?robot nil)
                    (param-types     rs  rs        workpiece robot  order)
                    (param-quantified )
                    (lookahead-time 20);10)
                    (preconditions "
                        (and
                            (not (mps-side-free ?rs OUTPUT))
                            (not (mps-side-free ?other-rs OUTPUT))
...
(defrule goal-class-create-pickup-wp-cs
    "When both CS are blocked, pick up a workpiece from a cap station."
    (wm-fact (key refbox team-color) (value ?team-color))
    (wm-fact (key domain fact self args? r ?robot))

    (wm-fact (key domain fact mps-type args? m ?cs t CS))
    (wm-fact (key domain fact mps-type args? m ?other-cs&~?cs t CS))

    (not (goal-class (class PICKUP-WP) (meta mps ?cs)))
    =>
    (assert
        (goal-class (class PICKUP-WP)
                    (id (sym-cat PICKUP-WP- ?cs))
                    (meta mps ?cs)
                    (type ACHIEVE)
                    (sub-type SIMPLE)
                    (param-names     cs  other-cs  wp        robot  order)
                    (param-constants ?cs ?other-cs nil       ?robot nil)
                    (param-types     cs  cs        workpiece robot  order)
                    (param-quantified )
                    (lookahead-time 20);10)
                    (preconditions "
                        (and
                            (not (mps-side-free ?cs OUTPUT))
                            (not (mps-side-free ?other-cs OUTPUT))
...
(defrule goal-class-assert-goal-pickup-wp
    "If the preconditions of a PICKUP-WP goal class are satisfied, assert the goal."
    (declare (salience (+ 1 ?*SALIENCE-GOAL-FORMULATE*)))
    (goal (id ?parent) (class NO-PROGRESS) (mode FORMULATED))
    ;(goal (id ?urgent) (class URGENT) (mode FORMULATED))

    (goal-class (class ?class&PICKUP-WP) (id ?cid) (sub-type ?subtype) (lookahead-time ?lt))
    (pddl-formula (part-of ?cid) (id ?formula-id))
    (grounded-pddl-formula (formula-id ?formula-id) (is-satisfied ?sat) (promised-from ?from) (promised-from-goals $?promised-from-goals) (grounding ?grounding-id))
    (pddl-grounding (id ?grounding-id) (param-values ?mps ?other-mps ?wp ?robot ?order))

    (not (goal (class ?class) (params mps ?mps)))
    (promise-time (usecs ?game-time))
    (test (sat-or-promised ?sat ?game-time ?from ?lt))

    ;check if the next step is possible (i.e. cap is buffered or ring payments were done)
    (wm-fact (key wp meta next-step args? wp ?wp) (value ?next-step))

    (or
        (and
            (test (eq ?next-step RING2))
            (wm-fact (key domain fact order-ring2-color args? ord ?order col ?ring2-color))
            (wm-fact (key domain fact rs-ring-spec args? m ?rs r ?ring2-color rn ?cost))
            (wm-fact (key domain fact rs-paid-for args? rs ?rs n ?cost))
        )
        (and
            (test (eq ?next-step RING3))
            (wm-fact (key domain fact order-ring3-color args? ord ?order col ?ring3-color))
            (wm-fact (key domain fact rs-ring-spec args? m ?rs r ?ring3-color rn ?cost))
            (wm-fact (key domain fact rs-paid-for args? rs ?rs n ?cost))
        )
        (and
            (test (eq ?next-step CAP))
            (wm-fact (key domain fact order-cap-color args? ord ?order col ?cap-color))
    ...
    ;assert promises resulting from the plan-action of this goal
    (assert
        (domain-promise (name mps-side-free) (param-values ?mps OUTPUT) (promising-goal ?goal-id) (valid-at (+ 8 ?game-time)) (negated FALSE))
    )
)
