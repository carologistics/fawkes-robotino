...
(defrule goal-expander-fill-cap-station-centralgoal-id
   ...
   (assert (wm-fact (key meta fact plan required-resources args? id ?plan-id)
                    (values ?cs ?cc))
           (wm-fact (key meta fact plan start-location args? id ?plan-id)
                    (values (wait-pos ?cs INPUT)  WAIT))
           (wm-fact (key meta fact plan end-location args? id ?plan-id)
                    (values (wait-pos ?cs INPUT)  WAIT))
         (wm-fact (key meta fact plan duration args? id ?plan-id) (type INT) (value 0))
         (wm-fact (key meta fact plan durative-action args? id ?plan-id) (is-list TRUE))
   )
   (assert
...
(defrule goal-produce-c0-central
 ...
 (not (plan (goal-id ?goal-id)))
 =>
 (bind ?plan-id  (sym-cat MOUNT-CAP- (gensym*)))
 (assert (wm-fact (key meta fact plan required-resources args? id ?plan-id)
                  (values ?wp ?bs ?cs))
         (wm-fact (key meta fact plan start-location args? id ?plan-id)
