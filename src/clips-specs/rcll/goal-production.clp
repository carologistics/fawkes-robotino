...
(defrule goal-production-create-fill-rs-for-order
  " For a started order that requires a ring, Create a PREPARE-RINGS-FOR-ORDER goal
    a RUN-ALL compound goal will be creatred under DO-FOR-ORDER.

     The PREPARE-RINGS-FOR-ORDER compound goal will attemet to get material
     to fill the rs from anywhere (via GET-TO-FILL-RS run-one goal). Then Upon
     evaluation of that getting-goal a FILL-RS goal will be created to fill
     the ring station.

     This introduces a new resource-Lock, ?rs-FILL owned by the
     PREPARE-FOR-ORDER goal, this will prevent other bots to start the early
     filling, This should later be generalized to all filling operations.

     It also groups all GET-FOR-FILL operations under a single RUN-ONE parent
    (GET-FOR-FILL)
"
