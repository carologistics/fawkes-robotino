...
"
  ?p <- (plan-action (plan-id ?plan-id) (goal-id ?goal-id)
	         ...
	         (action-name ?action-name)
	         (param-values $?param-values)
           (precondition ?precon-id))
  (pddl-grounding (id ?precon-id))
  (plan (id ?plan-id) (goal-id ?goal-id))
  (goal (id ?goal-id) (mode DISPATCHED))
  ...
  (do-for-all-facts ((?dg grounded-pddl-predicate)) (eq ?dg:grounding ?precon-id)
    (do-for-fact ((?dp pddl-predicate)) (eq ?dp:id ?dg:predicate-id)
      (if (and ?dg:is-satisfied
          (any-factp ((?df pddl-formula)) (and (eq ?df:id ?dp:part-of) (eq ?df:type negation))))
        then
        (printout error "Precondition (not " ?dp:predicate ?param-values ") is not satisfied." crlf)
      )
      (if (and (not ?dg:is-satisfied)
          (any-factp ((?df pddl-formula)) (not (and (eq ?df:id ?dp:part-of) (eq ?df:type negation)))))
        then
        (printout error "Precondition " ?dp:predicate ?param-values " is not satisfied." crlf)
      )
    )
  )
...
"
  (plan-action (plan-id ?plan-id) (goal-id ?goal-id)
     ...
     (precondition ?grounding-id))
  (grounded-pddl-predicate (grounding ?grounding-id) (predicate-id ?predicate-id))
  (pddl-predicate (id ?predicate-id) (predicate mps-side-free) (param-names ?mps-name ?side-name))
  (pddl-grounding (id ?grounding-id)
                  (param-values $?param-values)
                  (param-names $?param-names&:(and
                    (eq ?mps (domain-param-value-for-name ?param-values ?param-names ?mps-name))
                    (eq ?side (domain-param-value-for-name ?param-values ?param-names ?side-name))
                  )))
  (plan (id ?plan-id) (goal-id ?goal-id))
  (goal (id ?goal-id) (mode DISPATCHED))
