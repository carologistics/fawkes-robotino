...
=>
 ...
 (bind ?limited-resources (create$))
 (do-for-all-facts ((?srf schedule-resource)) (and (neq ?srf:resource-id Upsilan)
                                                   (neq ?srf:resource-id Set))
   (bind ?limited-resources (create$ ?limited-resources (sym-cat ?srf:resource-id)))
 )
 (scheduler-generate-model-for-resources (sym-cat ?s-id) ?limited-resources)
