...
" Fill a cap into a cap station.
  ...
  (assert (goal (id gPREPCS)
                (class PREPARE-CS) (sub-type SCHEDULE-SUBGOALS)
...
" Fill a cap into a cap station.
 ...
 ; (wm-fact (key domain fact cs-can-perform args? m ?mps op RETRIEVE_CAP))
  ...
  (assert (goal (id gFILLCAP)
                (class FILL-CAP) (sub-type SCHEDULE-SUBGOALS)
                (parent ?prepare-cs)
                (params (create$ cc ?cc cs ?cs))
				))
  (assert (goal (id gCLEARMPS)
                (class CLEAR-MPS) (sub-type SCHEDULE-SUBGOALS)
                (parent ?prepare-cs)
                (params (create$ wp ?cc mps ?cs side OUTPUT))
  ))
...
" Fill a cap into a cap station.
 ...
 ; (wm-fact (key domain fact cs-can-perform args? m ?mps op RETRIEVE_CAP))
  ...
  (assert (goal (id gMOUNTCAP)
                (class MOUNT-CAP) (sub-type SCHEDULE-SUBGOALS)
                (parent ?wp-operations)
                (params (create$ wp ?wp cs ?cs))
  ))
  (assert (goal (id gDELIVER)
                (class DELIVER) (sub-type SCHEDULE-SUBGOALS)
                (parent ?wp-operations)
                (params (create$ wp ?wp cs ?cs))
  ))
)
...
"
  ...
  (assert (goal (id gPRODUCEC0)
                (class PRODUCE-C0) (sub-type SCHEDULE-SUBGOALS)
                (parent ?mount-cap)
                (params robot ?robot
                        bs ?bs
                        bs-side ?bs-side
