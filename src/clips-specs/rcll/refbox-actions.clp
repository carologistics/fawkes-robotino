...
(defrule action-send-beacon-signal
  ...
  (bind ?ptime ?now)
  (if (not (do-for-fact ((?pose Position3DInterface)) (eq ?pose:id "Pose")
  ...
  (switch ?mps-type
    ...
    (case SS
      then
       ...
       (switch ?an
         (case prepare-ss-to-assign-wp then (bind ?instruction CHANGE_INFO))
         (case prepare-ss-to-store then (bind ?instruction STORE))
