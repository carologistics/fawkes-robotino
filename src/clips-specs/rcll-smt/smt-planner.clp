...
(defrule production-no-call-clips-smt-r-2
	...
	(wm-fact (key plan-action ?goal-id all-plan-actions-asserted))
=>
	(printout t "Expand for R-2" crlf)
...
(defrule production-no-call-clips-smt-r-3
	...
	(wm-fact (key plan-action all-plan-actions-asserted))
=>
	(printout t "Expand for R-3" crlf)
	(modify ?g (mode EXPANDED))
	(bind ?plan-id SMT-PLAN)
	(assert
		(plan (id ?plan-id) (goal-id ?goal-id))
...
(defrule production-smt-plan-completed
	...
	(assert (wm-fact (key plan-action COMPLEXITY all-plan-actions-asserted)))
)
