...
void VisualPositionThread::loadConfig(){
	...
	cfg_frame_camera_pos_ = config->get_string((cfg_prefix_static_transforms_ + "child_frame").c_str());
...
void VisualPositionThread::init_with_config()
{
	...
	if(cam_ == NULL){
		...
		if (!shm_buffer_color_->is_valid() || !shm_buffer_gray_->is_valid() || !shm_buffer_gray_edgy_->is_valid()) {
					delete shm_buffer_color_;
					delete shm_buffer_gray_;
					delete cam_;
					shm_buffer_color_ = NULL;
...
void VisualPositionThread::setup_color_classifier(color_classifier_context_t_ *color_data, const char* prefix, firevision::color_t expected)
{
...
VisualPositionThread::loop()
{
	if(!cfg_mutex_.try_lock()){
		...
		cam_->dispose_buffer();
		IplImage *p_image = cvCreateImage(cvSize(camera_info_.img_width_, camera_info_.img_height_), IPL_DEPTH_8U, 3);
...
//		std::vector<cv::Vec2f>::iterator lineshape_it = lines.begin();
		...
		if(myLines.size()==2){
			...
			cv::circle(image_colored, cv::Point(center.start.x -1, center.start.y -1), 3, cv::Scalar(255,0,0), 1);
...
//		edge_filter.set_src_buffer(buffer_gray_, &search_area, firevision::ORI_HORIZONTAL);
...
//		logger->log_info(name(),"found lines: %i %i", nrFirevision, model.getShapeCount());

//		std::vector<firevision::LineShape>* lineshapes = model.getShapes();

//		std::vector<firevision::ROI> rois;
//		for(std::vector<firevision::LineShape>::iterator lineshape_it = lineshapes->begin();
//				lineshape_it != lineshapes->end(); lineshape_it++)
//		{
//			int w = 10;
//			int h = 10;
//
...
//			int points[4];
//			(*lineshape_it).getPoints(&points[0], &points[1], &points[2], &points[3]);
//			r1.start.x = (unsigned int) points[0];
//			r1.start.y = (unsigned int) points[1];
//			r2.start.x = (unsigned int) points[2];
//			r2.start.y = (unsigned int) points[3];
//
...
void VisualPositionThread::calculatePuckPositions(std::vector<puck> *pucks, std::list<firevision::ROI> pucks_in_view){
...
void VisualPositionThread::printRoi(firevision::ROI roi){
	if(cfg_debugMessagesActivated_){
		logger->log_debug(name(),"x: %i y: %i width: %i height: %i color: %i image_hight: %i image_width: %i",
					roi.start.x,
					roi.start.y,
					roi.width,
					roi.height,
...
void VisualPositionThread::fitROI(firevision::ROI &roi, int x, int y, int w, int h){
	...
	if( roi.start.y + roi.height > roi.image_height || roi.start.x + roi.width > roi.image_width){
		logger->log_error(name(),"Failed fitRoi(x: %i y: %i w: %i h: %i) with roi: x: %i y:%i width: %i height: %i , image_width: %i image_height: %i",x, y, w, h, old.start.x, old.start.y, old.width, old.height, old.image_width, old.image_height);
		logger->log_error(name(),"Results Roi: x: %i y:%i width: %i height: %i , image_width: %i image_height: %i", roi.start.x, roi.start.y, roi.width, roi.height, roi.image_width, roi.image_height);
	}else if(cfg_debugMessagesActivated_){
		printRoi(roi);
	}
}

firevision::ROI* VisualPositionThread::getRoiContainingRoi( std::list<firevision::ROI>* roiList, firevision::ROI containing) {
...
firevision::ROI* VisualPositionThread::getBiggestRoi( std::list<firevision::ROI>* roiList) {
...
void VisualPositionThread::cartToPol(fawkes::polar_coord_2d_t &pol, float x, float y) {
	pol.phi = atan2f(y, x);
	pol.r = sqrtf(x * x + y * y);
...
void VisualPositionThread::polToCart(float &x, float &y,fawkes::polar_coord_2d_t pol){
	x = pol.r * std::cos(pol.phi);
	y = pol.r * std::sin(pol.phi);
}

void
VisualPositionThread::drawROIIntoBuffer(firevision::ROI roi, firevision::FilterROIDraw::border_style_t borderStyle)
{
	try{
		if(cfg_paintROIsActivated_){
			drawer_->set_src_buffer(buffer_color_, firevision::ROI::full_image(camera_info_.img_width_, camera_info_.img_height_), 0);
...
VisualPositionThread::classifyInRoi(firevision::ROI searchArea, color_classifier_context_t_* color_data)
{
	try{
		color_data->scanline_grid->reset();
		color_data->scanline_grid->set_roi(&searchArea);
		color_data->classifier->set_src_buffer(buffer_color_, camera_info_.img_width_, camera_info_.img_height_);
	...
	return new std::list<firevision::ROI>; // return a empty list if a exception is thrown
}
...
firevision::ROI VisualPositionThread::getCenterRoi(firevision::ROI p1, firevision::ROI p2){
...
void VisualPositionThread::config_value_changed(const fawkes::Configuration::ValueIterator *v)
{
...
Point3d VisualPositionThread::apply_tf(const char* target_frame,
		Point3d src) {
