...
RobotinoMachineDetectionThread::RobotinoMachineDetectionThread() :
		Thread("RobotinoMachineDetectionThread", Thread::OPMODE_WAITFORWAKEUP), VisionAspect(
				VisionAspect::CYCLIC) {
}
...
void RobotinoMachineDetectionThread::init() {
	...
	cam_ =
			vision_master->register_for_camera(
					"v4l2:front:device=/dev/video0:format=YUYV:size=160x120:white_balance_temperature_auto=0:exposure_auto=1:exposure_absolute=305",
					this);
	...
	shm_buffer_ = new SharedMemoryImageBuffer("machinedetection",
			pic_colorspace_, pic_width_, pic_height_);
	buffer_ = shm_buffer_->buffer();

	scanlinegrid_ = new ScanlineGrid(pic_width_, pic_height_, 5, 5,
			full_image_roi_);
...
void RobotinoMachineDetectionThread::loop() {
	...
	while (!scanlinegrid_->finished()) {
		unsigned char pix = buffer_[(*scanlinegrid_)->y * pic_width_
				+ (*scanlinegrid_)->x];
		if (pix > 200) {
	...
	for (std::list<Blob*>::iterator it = blobs.begin(); it != blobs.end();
		...
		logger->log_debug(name(), "Blob:(%d|%d)/(%d|%d)", (*it)->getLeftX(),
				(*it)->getUpperY(), (*it)->getRightX(), (*it)->getLowerY());
	}


	ImageDisplay * dis = new ImageDisplay(pic_width_, pic_height_,
			"buffer_filtered");
