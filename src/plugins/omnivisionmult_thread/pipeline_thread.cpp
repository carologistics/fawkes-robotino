...
RobotinoOmniVisionPipelineThread::sortFunctor::sortFunctor(
		firevision::RelativePositionModel * rp,
		firevision::SimpleColorClassifier * c) :
		relpos(rp), classifier(c) {
}
bool RobotinoOmniVisionPipelineThread::sortFunctor::operator()(
...
/** Lock data if fresh.*/
bool RobotinoOmniVisionPipelineThread::lock_if_new_data() {
	_data_mutex->lock();
	if (_new_data) {
		return true;
	} else {
...
/** Unlock data, */
void RobotinoOmniVisionPipelineThread::unlock() {
	_new_data = false;
	_data_mutex->unlock();
}
