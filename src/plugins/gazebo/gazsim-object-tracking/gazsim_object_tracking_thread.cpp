...
void ObjectTrackingThread::init()
{
	...
	puck_size_ = double(config->get_float(
	  "plugins/object_tracking/puck_values/puck_size"));
	puck_height_ = double(config->get_float(
	  "plugins/object_tracking/puck_values/puck_height"));

	belt_height_ = double(config->get_float(
	  "plugins/object_tracking/belt_values/belt_height"));
	belt_lenght_ = double(config->get_float(
	  "plugins/object_tracking/belt_values/belt_lenght"));
	belt_offset_side_ = double(config->get_float(
	  "plugins/object_tracking/belt_values/belt_offset_side"));

	slide_offset_side_ = double(config->get_float(
	  "plugins/object_tracking/slide_values/slide_offset_side"));
	slide_height_ = double(config->get_float(
	  "plugins/object_tracking/slide_values/slide_height"));

	left_shelf_offset_side_ = double(config->get_float(
	  "plugins/object_tracking/shelf_values/left_shelf_offset_side"));
	middle_shelf_offset_side_ = double(config->get_float(
	  "plugins/object_tracking/shelf_values/middle_shelf_offset_side"));
	right_shelf_offset_side_ = double(config->get_float(
	  "plugins/object_tracking/shelf_values/right_shelf_offset_side"));
	shelf_height_ = double(config->get_float(
	  "plugins/object_tracking/shelf_values/shelf_height"));

	gripper_offset_ = double(config->get_float(
	  "plugins/object_tracking/target_frame_offsets/gripper_offset"));
	base_offset_ = double(config->get_float(
	  "plugins/object_tracking/target_frame_offsets/base_offset"));
}
...
void ObjectTrackingThread::start_tracking(bool grab,
                                          std::string mps_name,
                                          std::string mps_side)
{
	...
	for(int i = 0; i < 14; i++)
	{
		...
		if(!mps_name.compare(mps_names_[i]))
		{
			...
			logger->log_info(mps_name.c_str(),
			                 std::to_string(mps_positions_[i][0]).c_str());
			logger->log_info(mps_name.c_str(),
			                 std::to_string(mps_positions_[i][1]).c_str());
			logger->log_info(mps_name.c_str(),
	...
	{
		middle_x_ = mps_x_ + belt_offset_side_  * cos(mps_ori_)
		        + (belt_lenght_ / 2 - puck_size_) * sin(mps_ori_);
		middle_y_ = mps_y_ + belt_offset_side_  * sin(mps_ori_)
		        - (belt_lenght_ / 2 - puck_size_) * cos(mps_ori_);
		middle_z_ = belt_height_ + puck_height_ / 2;
	}
...
double ObjectTrackingThread::compute_middle_x(double x_offset)
{
	return mps_x_ + x_offset * cos(mps_ori_) -
	       (belt_lenght_ / 2 - puck_size_) * sin(mps_ori_);
}

double ObjectTrackingThread::compute_middle_y(double y_offset)
{
	return mps_y_ + y_offset * sin(mps_ori_) +
	       (belt_lenght_ / 2 - puck_size_) * cos(mps_ori_);
}

double ObjectTrackingThread::distance_middle_to_puck(int puck_ind)
{
