...
##Old constraint: here we were still saying that the robot could decide not to move
##DOmain specific knowledge: paths are shorter if robots move! we then modified the const
##as below

####       ###Constraint for the first step: either we move to the first location or we stay
####    for i in range(1,n_robots+1):
####        v1 = varPos['pos_{}_1'.format(i)]
####        v2 = varD['d_{}_{}'.format(i,n_machines)]
####        # c1 == does not move
####        c1 = And(v1 == -4,v2 == 0)
####        # c2 == does move somewhere
####        c2 = []
####        for k in range(1,n_machines+1):
####            v2 = varD['d_{}_1'.format(i)]
####            v3 = distances['dist_0_{}'.format(k)]
####            #v3 = varDist['dist_0_{}'.format(k)]
...
################################################
def simpleOptimize(o, varD, n_machines):
    res = sat
...
##def z3Optimize(o, varD, n_machines):
    v1 = varD['d_1_{}'.format(n_machines)]
    v2 = varD['d_2_{}'.format(n_machines)]
    v3 = varD['d_3_{}'.format(n_machines)]
...
##          print(k,m[v])
    ...
    if(opt == 3):
        ...
        start = time.time()
        result = simpleOptimize(o, varD, n_machines)
        end = time.time()
