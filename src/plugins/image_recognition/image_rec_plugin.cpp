 ...
 *   */
class ImageRecognitionPlugin : public fawkes::Plugin
{
	 public:
	   /** Constructor.
	   *    * @param config Fawkes configuration
	   *       */
	    ImageRecognitionPlugin(Configuration *config)
	     : Plugin(config)
	    {
	       thread_list.push_back(new ImageRecognitionThread());
	    }
};

PLUGIN_DESCRIPTION("Plugin for recognizing machines using the RealSense data")
EXPORT_PLUGIN(ImageRecognitionPlugin)
