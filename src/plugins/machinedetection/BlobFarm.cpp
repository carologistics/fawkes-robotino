...
BlobFarm::BlobFarm(unsigned char* buffer, int buffer_width, int buffer_height,
		...
		buffer_(buffer), buffer_width_(buffer_width), buffer_height_(
				buffer_width), min_diam_(min_diam), max_diam_(max_diam), merge_distance_(
				merge_distance), threshold_(threshold) {
	// TODO Auto-generated constructor stub
}

void BlobFarm::add_Blob(Blob* blob) {
	blobs.push_back(blob);
}

void BlobFarm::add_Blob(int x, int y) {
	Blob* blob = new Blob(buffer_, buffer_width_, buffer_height_, threshold_, x,
			y);
	blobs.push_back(blob);
}
...
Blob * BlobFarm::merge_blob(Blob * first, Blob * second) {
	...
	Blob* merged = new Blob(buffer_, buffer_width_, buffer_height_, threshold_,
			x, y);
