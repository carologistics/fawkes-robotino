...
struct myLine{
	...
	void drawCV(cv::Mat image, cv::Scalar color = cv::Scalar(0,0,255)){
		cv::line( image, cv::Point(upper_point.start.x, upper_point.start.y), cv::Point(lower_point.start.x, lower_point.start.y), color, 3, CV_AA);
	}
};
...
private:
	...
	unsigned char *buffer_color_;													//reference to the buffer of shm_buffer_YCbCr (to use in code)
	unsigned char *buffer_gray_;													//reference to the buffer of shm_buffer_YCbCr (to use in code)
