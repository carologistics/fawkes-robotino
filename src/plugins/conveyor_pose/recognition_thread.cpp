...
void RecognitionThread::loop()
{
  ...
  if (iterations_++ >= cfg_icp_min_loops_ || epsilon_reached) {
    ...
    LaserLineInterface *best_ll;
    main_thread_->laserline_get_best_fit(best_ll);
    ...
    if(!best_ll){
      ...
      try {
        ...
        rotation_ok = yaw_diff <= 15.0/180.0*M_PI;
    ...
    if (hypot_mask[0] && icp_.getFitnessScore() < last_raw_fitness_ && rotation_ok) {
      // Match improved
