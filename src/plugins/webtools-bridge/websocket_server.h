...
public:
    ...
    void    on_message(websocketpp::connection_hdl hdl, websocketpp::server<websocketpp::config::asio>::message_ptr web_msg);
    
    void    run(uint16_t port);

private:
    std::map < websocketpp::connection_hdl, std::shared_ptr<WebSession>, std::owner_less<websocketpp::connection_hdl> >        hdl_ids_;
