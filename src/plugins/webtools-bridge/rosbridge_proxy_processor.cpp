...
/***************************************************************************
 *  rosbridge_proxy_processor.cpp - Processes  Requests Targeted for ROS. Proxy To RosBridge server
 *
...
RosBridgeProxyProcessor::process_request(std::shared_ptr <WebSession> web_session , std::string jsonMsg)
{
    ...
    it_peers_ = std::find_if(   peers_.begin()
                            ,   peers_.end()
                            ,   [&](const std::shared_ptr <ProxySession>  & v)
                                {
                                    return v->get_web_session().get() == web_session.get();
    ...
    if( it_peers_  == peers_.end() )
    {
        ...
        {//Give the connections thread time to try to connect
            ...
            if(finalized_) { logger_->log_warn("RosBridgeProxyProcessor"," ProxyServer down while connecting" ); return; }
                                        ...
                                        , std::shared_ptr<WebSession> web_session)
{
    ...
    {
        logger_->log_info("Processor:" , "Failed to advertise to '%s': %s", topic_name.c_str(), e.what());
        throw e;
    }
