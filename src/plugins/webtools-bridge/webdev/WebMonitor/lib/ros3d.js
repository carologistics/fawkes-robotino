...
ROS3D.MarkerArrayClient = function(options) {
  ...
  arrayTopic.subscribe(function(arrayMessage) {
    ...
    arrayMessage.markers.forEach(function(message) {
      ...
      //  console.table([updated],[message.ns , message.id]);
...
//         if(message.ns == "navgraph"  && !updated){
//          console.table([updated],[message.ns , message.id]);
...
//         }
        ...
        if(!updated) { // "ADD"
          ...
          if(message.ns == "navgraph" && message.type == 9 )
          {
              if( message.text. split('-') [0] == 'C' || message.text. split('-') [0] == 'M' )
              {
                ...
                ori_x: message.pose.orientation .x ,
                ori_y: message.pose.orientation . y ,
                ori_z: message.pose.orientation . z ,
...
// 0.69 0.35 0.01
              ...
              orientation_vector .z = 0;

              message_edited .mesh_resource = "meshes/mps_base.dae";
              message_edited . mesh_use_embedded_materials = true;


            var quaternion = new THREE.Quaternion();
            quaternion.setFromAxisAngle( new THREE.Vector3( 0, 0 , 1 ),  (Math.atan2(orientation_vector.y, orientation_vector.x) /2) );

            message_edited .pose.orientation = quaternion;
            ...
            var newMarker = new ROS3D.Marker({
                ...
                that.markers[machine_name] = new ROS3D.SceneNode({
                  frameID : message_edited.header.frame_id,
                  tfClient : that.tfClient,
                  object : newMarker
                });
                that.rootObject.add(that.markers[machine_name]);
            }
          }
        }
