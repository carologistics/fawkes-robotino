...
void ProtobufThead::init()
{
  message_register_ = new MessageRegister(proto_dirs());

  if (!bb_manager_)
    throw fawkes::Exception("BUG: %s's reference to blackboard manager thread hasn't been initialized", name());
}



bool ProtobufThead::pb_queue_incoming()
{
...
ProtobufThead::incoming_message ProtobufThead::pb_queue_pop()
{
  fawkes::MutexLocker lock(&msgq_mutex_);
 ...
 */
long int
ProtobufThead::peer_create_local_crypto(const std::string &address, int send_to_port, int recv_on_port,
                                        const std::string &crypto_key, const std::string &cipher)
{
  if (recv_on_port <= 0)  recv_on_port = send_to_port;

  if (send_to_port > 0) {
    protobuf_comm::ProtobufBroadcastPeer *peer =
        new protobuf_comm::ProtobufBroadcastPeer(
          address, send_to_port, recv_on_port,
    ...
    peer->signal_received()
        .connect(boost::bind(&ProtobufThead::handle_peer_msg, this, peer_id, _1, _2, _3, _4));
    peer->signal_recv_error()
        .connect(boost::bind(&ProtobufThead::handle_peer_recv_error, this, peer_id, _1, _2));
    peer->signal_send_error()
        .connect(boost::bind(&ProtobufThead::handle_peer_send_error, this, peer_id, _1));
 ...
 */
long int
ProtobufThead::peer_create_crypto(const std::string &address, int port,
                                  const std::string &crypto_key, const std::string &cipher)
{
 ...
 */
long int
ProtobufThead::peer_create_local(const std::string &address, int send_to_port,
                                 int recv_on_port)
{
...
void
ProtobufThead::send(long int peer_id,
