...
: Thread("VelocityShareThread", Thread::OPMODE_CONTINUOUS), ConfigurationChangeHandler(CFG_PREFIX)
{
}
...
VelocityShareThread::init()
{
	...
	path_sub_ = rosnode->subscribe("/move_base/GlobalPlanner/plan",
	                               1,
	                               &VelocityShareThread::pathCallback,
	                               this);
...
VelocityShareThread::loop()
{
	...
	if (update_needed_) {
		...
		unsigned int num_cells =
		  std::min(cfg_max_cell_lookahead_count_, (unsigned int)path_.poses.size());
...
void
VelocityShareThread::config_value_erased(const char *path)
{
}
void
VelocityShareThread::config_tag_changed(const char *new_tag)
{
}
void
VelocityShareThread::config_comment_changed(const Configuration::ValueIterator *v)
{
}
...
void
VelocityShareThread::load_config()
{
	...
	try {
		...
		logger->log_error(name(),
		                  "Can't read robot_vel_topic name. Setting default to %s",
	...
	try {
		...
		logger->log_error(name(),
		                  "number_of_segments config value is missing. Setting default to %u",
	...
	try {
		...
		logger->log_error(name(),
		                  "max_cell_lookahead_count config value is missing. "
		                  "Setting default to %u",
		                  cfg_max_cell_lookahead_count_);
	}

	// recreate an object for this robot in robot_memory,
	// we do not know the data structure of the previous object so simply create a
	// new one.
	mongo::BSONObjBuilder q;
	q.append("object", "robot");
	q.append("robot_number", robot_number_);
