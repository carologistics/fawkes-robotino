...
LaserFrontDistThread::init()
{
	...
	//open interfaces
	if_laser_ = blackboard->open_for_reading<Laser360Interface>(config->get_string("plugins/laser-front-dist/input_laser_interface").c_str());
	if_result_ = blackboard->open_for_writing<Position3DInterface>(config->get_string("plugins/laser-front-dist/output_result_interface").c_str());
}
...
LaserFrontDistThread::loop()
{
	...
	{
		if(!isnormal(if_laser_->distances(i)))
		{
			//this is invalid
	...
	{
		if(!isnormal(if_laser_->distances(359-i)))
		{
			//this is invalid
			if_result_->set_visibility_history(-1);
			if_result_->write();
