 ...
 */
TagPositionInterfaceHelper::TagPositionInterfaceHelper(fawkes::Position3DInterface *position_interface, u_int32_t vector_position, fawkes::BlackBoard *blackboard) :
  TransformAspect(TransformAspect::ONLY_PUBLISHER, (std::string("tag_")+std::to_string(vector_position)).c_str()),
  ClockAspect()
{
  ...
  this->clock = fawkes::Clock::instance();
  this->init_ClockAspect(this->clock);
  this->tf_publisher = new fawkes::tf::TransformPublisher(blackboard,(std::string("tag_")+std::to_string(vector_position)).c_str());
}
