...
local TOLERANCE_VS = {x = 0.02, y = 0.0325, ori = 0.015}
...
if config:exists("/plugins/laser-front-dist/target_frame") then
    laser_front_dist_frame = config:get_string(
                                 "/plugins/laser-front-dist/target_frame")
end

function invalid_params(self)
    return
        self.fsm.vars.ori <= -2 * math.pi or self.fsm.vars.ori >= 2 * math.pi or
...
function set_speed(self)
    ...
    local dist_target = tfm.transform6D(self.fsm.vars.target,
        ...
        for k, _ in pairs(dist_target) do
            ...
            if ((k == "x" or k == "y") and not fsm.vars.rotating) or
                ...
                if math.abs(scalar(dist_target[k])) >
                    self.fsm.vars.tolerance_arg[k] then
                    if D_DECEL[k] > 0 then
                        a[k] = V_MAX[k] / D_DECEL[k]
                    end
                    ...
                    v_dec = a[k] / self.fsm.vars.decel_factor *
                                math.abs(scalar(dist_target[k])) *
                                math.min(1,
                                         (2000 /
                                             (fsm.vars.loop_time["l"] +
                                                 fsm.vars.loop_time["ll"])) /
                                             DESIRED_HZ)

                    -- decide if we wanna decelerate, accelerate or max out
                    v[k] = math.min(self.fsm.vars.vmax_arg[k], math.max(
                                        V_MIN[k],
                    ...
                    if scalar(dist_target[k]) < 0 then
                        v[k] = v[k] * -1
                    end
                else
                    v[k] = 0
    ...
    if self.fsm.vars.frame and self.fsm.vars.target_frame ~= "/odom" then
        ...
        if tgt_odom then self.fsm.vars.fallback_target_odom = tgt_odom end
    end
...
function close_enough(self)
    local dist_target = tfm.transform6D(self.fsm.vars.target,
                                        self.fsm.vars.target_frame, "/base_link")
    return
        math.abs(dist_target.x) < TOLERANCE_EE.x and math.abs(dist_target.y) <
            TOLERANCE_EE.y and math.abs(scalar(dist_target.ori)) <
            TOLERANCE_EE.ori
end

function early_endable(self)
    return self.fsm.vars.end_early and close_enough(self) and
               self.fsm.vars.consecutive_detections > 2
end
...
function pos3d_iface(frame)
    ...
    if frame == laser_front_dist_frame then return if_front_dist end
    return false
end
...
function object_tracker_active() return object_tracking_if:has_writer() end
...
fsm:add_transitions{
    ...
    {"INIT", "DRIVE", cond = true}, {
        "WAIT_OTI",
        "DRIVE_VS",
        cond = "vars.visual_servoing and object_tracker_active()"
    ...
    {"DRIVE", "FINAL", cond = "vars.end_early and early_endable(self)"}, {
        "DRIVE",
        "WAIT_TRACKING",
        cond = "vars.end_early and drive_done(self) and not vars.dry_run"
    }, {"DRIVE", "FINAL", cond = drive_done}, {
        "WAIT_TRACKING",
        "FINAL",
        cond = early_endable,
        desc = "Target close enough and object detected"
    ...
    }, {"DRIVE_VS", "FAILED", cond = "vars.tf_failed", desc = "dist TF failed"},
    {
        ...
        desc = "Object cannot be found"
    }, {"DRIVE_VS", "FINAL", cond = drive_done}, {
        "DRIVE_CAM",
        "FALLBACK_TO_ODOM",
        cond = "not cam_frame_visible(vars.frame)",
        desc = "Lost frame"
    }, {
        "DRIVE_CAM",
        "FALLBACK_TO_ODOM",
    ...
    {"FALLBACK_TO_ODOM", "DRIVE", cond = true}, {
        "DRIVE",
        "RECOVER_TO_FRAME",
        cond = "cam_frame_visible(vars.frame)",
        desc = "frame visible again"
    }, {
        "RECOVER_TO_FRAME",
        "FAILED",
        cond = "vars.num_fallbacks > 5",
        desc = "Too many fallbacks"
...
function INIT:init()
    ...
    if self.fsm.vars.dry_run == nil then self.fsm.vars.dry_run = false end
    ...
    if self.fsm.vars.frame then
        ...
        }, "/base_link", self.fsm.vars.target_frame)
        if not cur_pos then
            print_error("Failed to transfrom base_link to " ..
    ...
    if self.fsm.vars.ori == math.pi then
        self.fsm.vars.ori = self.fsm.vars.ori - 1e-6
    end
    if self.fsm.vars.ori == -math.pi then
        self.fsm.vars.ori = self.fsm.vars.ori + 1e-6
    end
    ...
    if self.fsm.vars.frame then
        ...
        if not self.fsm.vars.fallback_target_odom then
            print_error("Failed to transfrom " .. self.fsm.vars.target_frame ..
                            " to odom")
        end
    ...
    printf("Target %s: %f, %f, %f, %f", self.fsm.vars.target_frame,
           self.fsm.vars.target.x, self.fsm.vars.target.y,
...
function DRIVE:init()
    ...
    print_info("motor_move tolerance x: %f, y: %f, ori: %f",
               self.fsm.vars.tolerance_arg.x, self.fsm.vars.tolerance_arg.y,
    ...
    if fsm.vars.end_early then
        if self.fsm.vars.msgid ~= object_tracking_if:msgid() then
            ...
            if object_tracking_if:is_detected() then
                self.fsm.vars.consecutive_detections = self.fsm.vars
...
function DRIVE_VS:loop()
    ...
    local v_x_dec = a_x / self.fsm.vars.decel_factor *
                        math.abs(if_front_dist:translation(0) - SAFE_DIST)
    self.fsm.vars.vmax_arg.x = math.min(V_MAX.x,
                                        self.fsm.vars.vel_trans or V_MAX.x,
                                        v_x_dec)
    ...
    if self.fsm.vars.msgid ~= object_tracking_if:msgid() then
        ...
        if object_tracking_if:is_detected() then
            ...
            self.fsm.vars.missing_detections =
                self.fsm.vars.missing_detections + 1
        end
    end

    self.fsm.vars.target.x = object_tracking_if:base_frame(0)
    self.fsm.vars.target.y = object_tracking_if:base_frame(1)
    self.fsm.vars.target.ori = fawkes.tf.create_quaternion_from_yaw(
                                   -object_tracking_if:base_frame(5))
    set_speed(self)
end
...
function FALLBACK_TO_ODOM:init()
    printf("motor_move: lost target frame %s, falling back to odom frame",
           self.fsm.vars.target_frame)
...
function RECOVER_TO_FRAME:init()
    printf("motor_move: found original target frame %s again, using it",
           self.fsm.vars.frame)
