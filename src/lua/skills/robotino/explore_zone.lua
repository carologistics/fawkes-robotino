...
depends_interfaces = {
    {
        v = "zone_info",
        type = "ZoneInterface",
        id = "/explore-zone/info",
        writing = true
    }, {
        v = "bb_found_tag",
        type = "Position3DInterface",
        id = "/explore-zone/found-tag",
        writing = true
...
local ZONE_CORNERS = {
    {x = 0.7, y = 0.07}, {x = -0.07, y = 0.7}, {x = -0.76, y = -0.81}
}

-- Maximum difference between tag and line trans/rot
local TAG_LINE_TOLERANCE = {trans = 0.2, rot = 0.05}
...
function in_zone(x, y)
    return math.abs(x - fsm.vars.x) < 0.5 - ZONE_MARGIN and
               math.abs(y - fsm.vars.y) < 0.5 - ZONE_MARGIN
end
...
function line_in_zone(lines)
    for k, line in pairs(lines) do
        if line:visibility_history() >= MIN_VIS_HIST then
            ...
            if in_zone(center_map.x, center_map.y) then return line end
        end
    end
    return nil
end
    ...
    for i, cluster in pairs(clusters) do
        if cluster:visibility_history() >= MIN_VIS_HIST then
            local cluster_map = tfm.transform({
                x = cluster:translation(0),
                y = cluster:translation(1),
                ori = 0
            }, cluster:frame(), "map")
...
function local_bearing(x, y)
    local p_bl = tfm.transform({x = x, y = y, ori = 0}, "map", "base_link")
    return math.atan2(p_bl.y, p_bl.x)
end

function found_tag()
    local tag_ifs = tag_utils.matching_tags(fsm.vars.tags, tag_info,
                                            fsm.vars.tag_id_set)
        ...
        if tag:visibility_history() >= MIN_VIS_HIST then
            ...
            if line and line:visibility_history() >= MIN_VIS_HIST then
                ...
                local line_c_tag = tfm.transform6D({
                    ...
                    y = line_c.y,
                    z = 0,
                    ori = fawkes.tf.create_quaternion_from_rpy(0, 0,
                                                               line:bearing() -
                                                                   math.pi)
                }, line:frame_id(), tag_utils.frame_for_id(fsm.vars.tags,
                                                           tag_info, id))
                ...
                if d_trans <= TAG_LINE_TOLERANCE.trans then
                    local line_bearing_map = tfm.transform({
                        x = 0,
                        y = 0,
                        ori = math.normalize_mirror_rad(line:bearing() + math.pi)
                    }, line:frame_id(), "map")
                    ...
                    if not tag_map then
                        ...
                        if in_zone(tag_map.x, tag_map.y) then
                            ...
                            if yaw == yaw then -- false for NAN
                                ...
                                local yaw_discrete =
                                    math.round(yaw / math.pi * 4) * 45
                                if yaw_discrete == 360 then
                                    yaw_discrete = 0
                                end
                                ...
                                local line_bearing_map_q = fawkes.tf
                                                               .create_quaternion_from_yaw(
                                                               line_bearing_map.ori)
                                bb_found_tag:set_rotation(0,
                                                          line_bearing_map_q:x())
                                bb_found_tag:set_rotation(1,
                                                          line_bearing_map_q:y())
                                bb_found_tag:set_rotation(2,
                                                          line_bearing_map_q:z())
                                bb_found_tag:set_rotation(3,
                                                          line_bearing_map_q:w())
                                bb_found_tag:set_frame("map")
                                bb_found_tag:set_visibility_history(
                                    tag:visibility_history())
                                fsm.vars.found_something = true
                                return true
                            end
                        else
                            printf(
                                "Discarding tag #%d, misaligned. Is at %f\t%f",
                                id, tag_map.x, tag_map.y)
                        end
                    end
                end
...
function lost_tag()
    local tag_ifs = tag_utils.matching_tags(fsm.vars.tags, tag_info,
                                            fsm.vars.tag_id_set)
    for id, tag in pairs(tag_ifs) do
        if tag:visibility_history() > -3 then return false end
    end
    return true
end
...
fsm:add_transitions{
    ...
    {"WAIT_FOR_SENORS", "WAIT_AMCL", cond = found_tag, desc = "found tag"}, {
        "WAIT_FOR_SENORS",
        "FAILED",
        cond = "vars.attempts >= MAX_ATTEMPTS",
        desc = "give up"
...
function INIT:init()
    ...
    if line then self.fsm.vars.line_center = llutils.center(line) end
end

function TURN:init()
    if self.fsm.vars.line_center then
        self.args["motor_move"].ori = math.atan2(self.fsm.vars.line_center.y,
                                                 self.fsm.vars.line_center.x)
    else
        self.args["motor_move"].ori = local_bearing(self.fsm.vars.x,
                                                    self.fsm.vars.y)
    end
end
...
function WAIT_FOR_SENORS:loop()
    ...
    if line then
        ...
        printf("p_map: %f, %f; ori = %f", p_map.x, p_map.y,
               fawkes.tf.get_yaw(p_map.ori))
        if within_map(p_map) then self.fsm.vars.line_vista = p_map end
    end
    ...
    if not self.fsm.vars.line_vista and cluster_map then
        ...
        local left_bl = {x = zone_bl.x - x, y = zone_bl.y + y, ori = 0}
        left_map = tfm.transform(left_bl, "base_link", "map")
        left_map.ori = math.atan2(self.fsm.vars.y - left_map.y,
                                  self.fsm.vars.x - left_map.x)
        if within_map(left_map) then
            self.fsm.vars.cluster_vista = left_map
            print_debug("left_map: " .. left_map.x .. ", " .. left_map.y ..
                            "; ori = " .. left_map.ori)
        else
            local right_bl = {x = zone_bl.x - x, y = zone_bl.y - y, ori = 0}
            right_map = tfm.transform(right_bl, "base_link", "map")
            right_map.ori = math.atan2(self.fsm.vars.y - right_map.y,
                                       self.fsm.vars.x - right_map.x)
            if within_map(right_map) then
                self.fsm.vars.cluster_vista = right_map
                print_debug(
                    "right_map: " .. right_map.x .. ", " .. right_map.y ..
                        "; ori = " .. right_map.ori)
            end
        end
    end
...
function PICK_VISTA_POINT:init()
    ...
    while not self.fsm.vars.zone_corner and self.fsm.vars.zone_corner_idx <=
        #ZONE_CORNERS do
        local dx = ZONE_CORNERS[self.fsm.vars.zone_corner_idx].x
        local dy = ZONE_CORNERS[self.fsm.vars.zone_corner_idx].y

        if math.abs(self.fsm.vars.x + dx) > X_MAX then dx = 0 end
        if self.fsm.vars.y + dy > Y_MAX or self.fsm.vars.y + dy < Y_MIN then
            dy = 0
        end
        if math.abs(self.fsm.vars.x + dx) > 4 and self.fsm.vars.y + dy < Y_MIN +
            1 then
            dx = 0
            dy = 0
        end
...
function GOTO_VISTA_POINT:init() self.args["moveto"] = self.fsm.vars.zone_corner end
...
function WAIT_AMCL:loop() found_tag() end
