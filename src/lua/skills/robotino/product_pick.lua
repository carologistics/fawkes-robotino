...
local conveyor_gripper_down_z = -0.027 -- distance to move gripper down after driving over product
...
if config:exists("/skills/product_pick/gripper_down_z") then
    conveyor_gripper_down_z = config:get_float(
                                  "/skills/product_pick/gripper_down_z")
end
...
function pose_not_exist()
    ...
    if transformed_pos == nil then return true end
    return false
end
...
fsm:add_transitions{
    ...
    {"INIT", "OPEN_GRIPPER", true, desc = "Open gripper for product_pick"},
    {
        ...
        "FAILED",
        cond = "not is_grabbed()",
        desc = "Don't hold puck!"
    }, -- add or not is_grabbed()
...
function MOVE_GRIPPER_FORWARD:init()
    local pose = {}
    pose = pose_gripper_offset(conveyor_gripper_forward_x, 0,
                               conveyor_gripper_down_z)
