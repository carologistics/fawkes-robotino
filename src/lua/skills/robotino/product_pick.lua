...
local gripper_tolerance_z = 0.5 -- gripper z tolerance according to conveyor pose
  ...
  local gripper_down_z = 0.027    -- distance to move gripper down after driving over product
...
local gripper_init_z = 0.03 -- initial z position of the gripper
  ...
  local gripper_pose_offset_z = 0.06  -- conveyor_pose offset in z direction

  local align_target_frame = "gripper"      -- the gripper align is made relative to this frame (according to gripper_commands)
  local z_movement_target_frame = "gripper_home" -- the gripper z movement is made relative to this frame (according to gripper_commands)
...
fsm:define_states{ export_to=_M, closure={gripper_if=gripper_if},
   {"INIT", JumpState},
   {"INIT_GRIPPER", SkillJumpState, skills={{gripper_commands}}, final_to="OPEN_GRIPPER", fail_to="CLEANUP_FAILED"},
   {"OPEN_GRIPPER", SkillJumpState, skills={{gripper_commands}},final_to="CHECK_VISION", fail_to="CLEANUP_FAILED"},
   {"CHECK_VISION", JumpState},
   {"GRIPPER_ALIGN", SkillJumpState, skills={{gripper_commands}}, final_to="CHECK_TOLERANCE",fail_to="CLEANUP_FAILED"},
   {"CHECK_TOLERANCE",JumpState},
   {"MOVE_GRIPPER_FORWARD", SkillJumpState, skills={{gripper_commands}}, final_to="MOVE_GRIPPER_DOWN",fail_to="CLEANUP_FAILED"},
   {"MOVE_GRIPPER_DOWN", SkillJumpState, skills={{gripper_commands}}, final_to="CLOSE_GRIPPER", fail_to="CLEANUP_FAILED"},
   {"CLOSE_GRIPPER", SkillJumpState, skills={{gripper_commands}}, final_to="MOVE_GRIPPER_DOWN_SECOND", fail_to="CLEANUP_FAILED"},
   {"MOVE_GRIPPER_DOWN_SECOND", SkillJumpState, skills={{gripper_commands}}, final_to="MOVE_GRIPPER_BACK", fail_to="CLEANUP_FAILED"},
   {"MOVE_GRIPPER_BACK", SkillJumpState, skills={{gripper_commands}}, final_to = "HOME_GRIPPER", fail_to="CLEANUP_FAILED"},
...
function INIT_GRIPPER:init()
  ...
  self.args["gripper_commands"].x = gripper_init_x
  self.args["gripper_commands"].y = gripper_init_y
  self.args["gripper_commands"].z = gripper_init_z
end
...
function GRIPPER_ALIGN:init()
  ...
  self.args["gripper_commands"].command = "MOVEABS"
  self.args["gripper_commands"].target_frame  = align_target_frame
end

function MOVE_GRIPPER_FORWARD:init()
  self.args["gripper_commands"].command = "MOVEABS"
  self.args["gripper_commands"].x = gripper_forward_x
  self.args["gripper_commands"].target_frame = x_movement_target_frame
end

function MOVE_GRIPPER_DOWN:init()
  self.args["gripper_commands"].command = "MOVEABS"
  self.args["gripper_commands"].z = gripper_down_z
  self.args["gripper_commands"].target_frame = z_movement_target_frame
end

function MOVE_GRIPPER_DOWN_SECOND:init()
  self.args["gripper_commands"].z = gripper_down_second_z
  self.args["gripper_commands"].target_frame = z_movement_target_frame
  self.args["gripper_commands"].command = "MOVEABS"
end
