...
Parameters:
      @param gripper the gripper mounted on the robot: (NEW_GRIPPER| OLD_GRIPPER)
...
Parameters:
      @param gripper mounted gripper: (NEW_GRIPPER | OLD_GRIPPER)
      @param place   the name of the MPS (see navgraph)
      @param side    optional the side of the mps, default is input (give "output" to bring to output)
      @param shelf   optional position on shelf: ( LEFT | MIDDLE | RIGHT )
      @param slide   optional true if you want to put it on the slide
...
fsm:define_states{ export_to=_M, closure={navgraph=navgraph,gripper_if=gripper_if},
   {"INIT", JumpState},
   {"OLD_BRING_PRODUCT", SkillJumpState, skills={{get_product_from}}, final_to="FINAL", fail_to="FAILED"},
   {"NEW_BRING_PRODUCT", SkillJumpState, skills={{get_product_from_new}}, final_to="FINAL", fail_to="FAILED"},
}
