...
local target_outputs = {
    {target = "WORKPIECE", mps = "C-CS1", side = "SHELF-LEFT"},
...
function PICK:init()
    ...
    self.args["manipulate_wp"] = {
        target = fsm.vars.next_target_output.target,
...
function PICK:exit()
    ...
    print_info(
        "[VS] Execution time for output " .. fsm.vars.current_output_id .. ": " ..
...
function PUT:init()
    ...
    self.args["manipulate_wp"] = {
        target = fsm.vars.next_target_input.target,
...
function PUT:exit()
    local now = fawkes.Time:new():in_msec()
    print_info("[VS] Execution time for input " .. fsm.vars.current_input_id ..
                   ": " .. now - fsm.vars.time_start)
end
