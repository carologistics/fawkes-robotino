...
fsm:define_states{
   ...
   {"PPGOTO", JumpState},
   {"TIMEOUT", JumpState},
   {"SKILL_GOTO", SkillJumpState, skills={{moveto}}, final_to="FINAL", fail_to="FAILED"},
}
...
fsm:add_transitions{
   ...
   {"PPGOTO", "TIMEOUT", cond=jumpcond_navifail, desc="Navigator failure with err: " .. ppnavi:error_code() .. " try moveto"},
...
function SKILL_GOTO:init()
   self.args["moveto"] = { x = self.fsm.vars.x, y = self.fsm.vars.y,
