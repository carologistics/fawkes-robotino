...
function has_writer()
   return (yolo_interface:has_writer() and
           ...
           realsense_boundingbox:has_writer() and 
           workpiece_pose_if:has_writer())
end
...
fsm:add_transitions{
   ...
   {"GET_XYZ", "FAILED", cond="vars.xyz_retries > 2", desc="Confindences too low or Bounding box out of range"},
   {"GET_XYZ", "DETECT_WORKPIECE", cond="vars.low_conf_or_out_of_range", desc="Taking new picture to look again"},
   {"GET_XYZ", "PUBLISH_POSE", cond="check_bounding_box_if_msgid(self)", desc="Deprojected pixel to point"},
   {"PUBLISH_POSE", "FAILED", cond="vars.pose_retries > 2", desc="Can't get XYZ, deprojection returned 0,0,0"},
   {"PUBLISH_POSE", "GET_XYZ", cond="vars.zero_pose", desc="Retrying deprojection"},
...
function INIT:init()
   ...
   self.fsm.vars.xyz_retries = 0
   self.fsm.vars.pose_retries = 0
...
function GET_XYZ:init()
   ...
   self.fsm.vars.xyz_retries = self.fsm.vars.xyz_retries + 1
...
function PUBLISH_POSE:init()
   ...
   self.fsm.vars.pose_retries = self.fsm.vars.pose_retries + 1
   if x == 0 and y == 0 and z == 0 then
      self.fsm.vars.zero_pose = true
   else
      wp_pose_msg = workpiece_pose_if.WPPoseMessage:new()
      wp_pose_msg:set_translation(0, x)
      wp_pose_msg:set_translation(1, y)
      wp_pose_msg:set_translation(2, z)
      workpiece_pose_if:msgq_enqueue(wp_pose_msg)
   end
end
