...
fsm:define_states { export_to=_M,
   {"INIT", JumpState},
   {"DRIVE", SkillJumpState, skills={{relgoto}}, final_to="FINAL", fail_to="FAILED"},
...
function DRIVE:init()
   -- relgoto
   tag = get_tag_with_id(self.fsm.vars.id)
   print("x: " .. tag:translation(2) - align_tag_param.x)
   print("y: " .. tag:translation(0) - align_tag_param.y)
   self.skills[1].x = tag:translation(2) - align_tag_param.x
   self.skills[1].y = tag:translation(0) - align_tag_param.y
end
