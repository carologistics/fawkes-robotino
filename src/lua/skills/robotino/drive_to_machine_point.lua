...
documentation =
    [==[Drive to point with colisoin avoidance and approach a point relative to the laser line.
...
local OFFSET_CONVEYOR = 0
local OFFSET_SHELF_RIGHT = -0.3
local OFFSET_SHELF_MIDDLE = -0.2
local OFFSET_SHELF_LEFT = -0.1
local OFFSET_SLIDE = -0.27

local MIN_VIS_HIST_LINE = 5 -- 15
local MIN_VIS_HIST_LINE_SEARCH = 6 -- 15

function node_is_valid(self)
    if self.fsm.vars.point_set then return self.fsm.vars.point_valid end
    return true
end
...
function match_line(self, lines)
    ...
    for k, line in pairs(self.fsm.vars.lines) do
        ...
        local d_navgraph_to_line = math.vec_length(
                                       navgraph_point_laser.x - line_center.x,
                                       navgraph_point_laser.y - line_center.y)

        -- this is difference from the laser to navgraph point
        local d_laser_to_navgraph = math.vec_length(navgraph_point_laser.x,
                                                    navgraph_point_laser.y)
        ...
        if line:visibility_history() >= MIN_VIS_HIST_LINE and d_navgraph_to_line <
            LINE_MATCH_TOLERANCE and d_laser_to_navgraph <
            NAVGRAPH_LIN_TOLERANCE and math.abs(yaw) < NAVGRAPH_ANG_TOLERANCE and
            ang_dist < LINE_MATCH_ANG_TOLERANCE then matched_line = line end
    end
    ...
    for k, line in pairs(good_lines) do
        if line then
            ...
            if line:visibility_history() >= MIN_VIS_HIST_LINE_SEARCH and
                ...
                LINE_LENGTH_MAX and self.fsm.vars.lines_visited[line:id()] <
                max_num_visited -- and
            then table.insert(rv, line) end
        end
    end
    return rv
end
...
function laser_line_found(self)
    ...
    if self.fsm.vars.matched_line ~= nil then
        ...
        self.fsm.vars.line_point = llutils.point_in_front(llutils.center(
                                                              self.fsm.vars
                                                                  .matched_line),
                                                          self.fsm.vars.x_at_mps)
    end
...
function laser_line_still_visible(self)
    if self.fsm.vars.matched_line ~= nil then
        return
            self.fsm.vars.lines[self.fsm.vars.matched_line:id()]:visibility_history() >=
                MIN_VIS_HIST_LINE_SEARCH
    else
        return false
    end
end
...
fsm:define_states{
    ...
    closure = {
        ...
        laser_line_still_visible = laser_line_still_visible,
        laser_line_found = laser_line_found,
        NUM_DIRECT_MPS_ALIGN_TRIES = NUM_DIRECT_MPS_ALIGN_TRIES
    },
...
fsm:add_transitions{
    ...
    {"INIT", "FAILED", cond = "not node_is_valid(self)", desc = "point invalid"},
    {
        "INIT",
        "FAILED",
        cond = "not parameters_valid(self)",
...
function INIT:init()
    ...
    if self.fsm.vars.place ~= nil then
        ...
        if self.fsm.vars.node:is_valid() then
            ...
            if self.fsm.vars.ori == nil and
                self.fsm.vars.node:has_property("orientation") then
                self.fsm.vars.ori = self.fsm.vars.node:property_as_float(
                                        "orientation");
            end
        end
    end
...
function SKILL_MPS_ALIGN:init()
    self.args["mps_align"] = {
        ...
        x = self.fsm.vars.x_at_mps,
        y = self.fsm.vars.move_y
    }
end
