...
function input_ok()
    local port_ok = fsm.vars.port or
                        (fsm.vars.send_to_port and fsm.vars.recv_on_port) and
                        not (fsm.vars.port and
                            (fsm.vars.send_to_port or fsm.vars.recv_on_port))
...
fsm:add_transitions{
    {
        "SEND_MSG",
        "FAILED",
        precond = "not input_ok()",
        desc = "wrong input params"
...
function SEND_MSG:init()
    ...
    if fsm.vars.send_to_port then
        if fsm.vars.cipher then
            msg = peer_iface.CreatePeerLocalCryptoMessage:new(fsm.vars.address,
                                                              fsm.vars
                                                                  .send_to_port,
                                                              fsm.vars
                                                                  .recv_on_port,
            ...
            msg = peer_iface.CreatePeerLocalMessage:new(fsm.vars.address,
                                                        fsm.vars.send_to_port,
                                                        fsm.vars.recv_on_port)
        end
    else
        if fsm.vars.cipher then
            msg = peer_iface.CreatePeerCryptoMessage:new(fsm.vars.address,
                                                         fsm.vars.port,
                                                         fsm.vars.crypto_key,
                                                         fsm.vars.cipher)
        else
            msg = peer_iface.CreatePeerMessage:new(fsm.vars.address,
                                                   fsm.vars.port)
        end
    end
