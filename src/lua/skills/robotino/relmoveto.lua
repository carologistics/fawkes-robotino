...
documentation =
    [==[Move to base_link coords using navigator. Fail on navigator error only.
...
function can_navigate() return navigator:has_writer() end
...
function MOVING:init()
    ...
    if not math.isnan(self.fsm.vars.ori) then
        local msg_ori = navigator.SetOrientationModeMessage:new(
                            navigator.OrientAtTarget)
