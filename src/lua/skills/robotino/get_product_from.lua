...
depends_skills = {
    "product_pick", "drive_to_machine_point", "conveyor_align", "shelf_pick"
}
depends_interfaces = {
...
function already_at_conveyor(self) return (self.fsm.vars.atmps == "CONVEYOR") end

function shelf_set(self) return self.fsm.vars.shelf ~= nil end

fsm:define_states{
    export_to = _M,
    closure = {navgraph = navgraph, shelf_set = shelf_set},
    {"INIT", JumpState},
    {
        "DRIVE_TO_MACHINE_POINT",
        SkillJumpState,
        skills = {{drive_to_machine_point}},
        final_to = "CONVEYOR_ALIGN",
        fail_to = "FAILED"
    },
    {
        "CONVEYOR_ALIGN",
        SkillJumpState,
        skills = {{conveyor_align}},
        final_to = "DECIDE_ENDSKILL",
        fail_to = "FAILED"
    },
...
fsm:add_transitions{
    {"INIT", "FAILED", cond = "not navgraph", desc = "navgraph not available"},
    {
        "INIT",
        "FAILED",
        cond = "not vars.node:is_valid()",
        desc = "point invalid"
    }, {
        "INIT",
        "CONVEYOR_ALIGN",
        cond = already_at_conveyor,
        desc = "Already in front of the mps, align"
...
function INIT:init()
    ...
    if self.fsm.vars.side == nil then self.fsm.vars.side = "output" end
end

function DRIVE_TO_MACHINE_POINT:init()
    self.fsm.vars.time_start = fawkes.Time:new():in_msec()
    local option = "CONVEYOR"
    if self.fsm.vars.shelf ~= nil then self.fsm.vars.side = "input" end
    if self.fsm.vars.side == "input" or self.fsm.vars.shelf then
        self.fsm.vars.tag_id = navgraph:node(self.fsm.vars.place)
                                   :property_as_float("tag_input")
        self.args["drive_to_machine_point"] = {
            place = self.fsm.vars.place .. "-I",
            option = option,
            x_at_mps = X_AT_MPS,
        ...
        self.fsm.vars.tag_id = navgraph:node(self.fsm.vars.place)
                                   :property_as_float("tag_output")
        self.args["drive_to_machine_point"] = {
            place = self.fsm.vars.place .. "-O",
            option = option,
            x_at_mps = X_AT_MPS,
            tag_id = self.fsm.vars.tag_id
        }
...
function DRIVE_TO_MACHINE_POINT:exit()
    ...
    print_info("[ICP] Positioning took " .. now - self.fsm.vars.time_start ..
                   " milliseconds")
end
...
function PRODUCT_PICK:exit()
    ...
    local now = fawkes.Time:new():in_msec()
    print_info("[ICP] Routine took " .. now - self.fsm.vars.time_start ..
                   " milliseconds")
end
...
function CONVEYOR_ALIGN:exit()
    ...
    local now = fawkes.Time:new():in_msec()
    print_info("[ICP] Alignment took " .. now - self.fsm.vars.time_start ..
                   " milliseconds")
end

function FAILED:init()
    if self.fsm.vars.error then self.fsm:set_error(self.fsm.vars.error) end
end
