...
depends_interfaces = {
    {v = "motor", type = "MotorInterface", id = "Robotino"}, {
        v = "if_conveyor_pose",
        type = "ConveyorPoseInterface",
        id = "conveyor_pose/status"
    },
    {
        v = "if_plane_switch",
...
function no_writer() return not if_conveyor_pose:has_writer() end

function input_ok()
    if fsm.vars.slide then return true end
    if fsm.vars.side == "input" or fsm.vars.side == "output" then return true end
    return false
end
...
function tolerance_ok()
    ...
    if not gripper_odom then return nil end

    local gripper_dist = tfm.transform6D(gripper_odom, "odom", "gripper")
    if not gripper_dist then return nil end

    local q = fawkes.tf.Quaternion:new(gripper_dist.ori.x, gripper_dist.ori.y,
                                       gripper_dist.ori.z, gripper_dist.ori.w)
    local yaw = fawkes.tf.get_yaw(q)

    print_info("gripper_dist: %f, %f, ,%f, %f", gripper_dist.x, gripper_dist.y,
               gripper_dist.z, yaw)
    ...
    return gripper_dist.x < gripper_max.x - GRIPPER_POSE.x and gripper_dist.x >
               gripper_min.x - GRIPPER_POSE.x and gripper_dist.y < 0.8 *
               (gripper_max.y - GRIPPER_POSE.y) and gripper_dist.y > 0.8 *
               (gripper_min.y - GRIPPER_POSE.y) and gripper_dist.z <
               gripper_max.z - GRIPPER_POSE.z and gripper_dist.z > gripper_min.z -
               GRIPPER_POSE.z and math.abs(yaw) <= 0.05
end

function motor_move_tolerance()
    -- Compute motor_move tolerance as a function of the available gripper range
    return {
        x = math.max(0.01, -- lower bound
        0.5 * math.min( -- gripper range
        math.abs(gripper_max.x - GRIPPER_POSE.x),
        math.abs(gripper_min.x - GRIPPER_POSE.x))),
        y = math.max(0.01, -- lower bound
        0.5 * math.min( -- gripper range
...
function fitness_min()
    return if_conveyor_pose:euclidean_fitness() >=
               FITNESS_THRESHOLD[fsm.vars.target].low
end

function fitness_high()
    return if_conveyor_pose:euclidean_fitness() >=
               FITNESS_THRESHOLD[fsm.vars.target].high
end

function fitness_low() return fitness_min() and not fitness_high() end

function result_ready()
    if if_conveyor_pose:is_busy() or if_conveyor_pose:msgid() ~= fsm.vars.msgid then
        return false
    end

    if if_conveyor_pose:msgid() == fsm.vars.msgid and
        not if_conveyor_pose:is_busy() then return true end

    local bb_stamp = fawkes.Time:new(if_conveyor_pose:input_timestamp(0),
                                     if_conveyor_pose:input_timestamp(1))
...
fsm:define_states{
    ...
    closure = {
        ...
        MAX_VISION_RETRIES = MAX_VISION_RETRIES,
        fitness_low = fitness_low,
        fitness_high = fitness_high
    },
    {"INIT", JumpState},
    {
        "MOVE_GRIPPER",
...
fsm:add_transitions{
    ...
    {"LOOK", "FAILED", cond = no_writer, desc = "No writer for conveyor vision"},
    {
        ...
        "LOOK_DONE",
        cond = result_ready,
        desc = "conveyor_pose result ready"
...
function INIT:init()
    ...
    local parse_result = pam.parse_to_type_target(if_conveyor_pose,
                                                  self.fsm.vars.place,
                                                  self.fsm.vars.side, nil,
                                                  self.fsm.vars.slide)
...
function LOOK:init()
    local msg = if_conveyor_pose.RunICPMessage:new(self.fsm.vars.mps_type,
                                                   self.fsm.vars.mps_target)
...
function LOOK:exit()
    ...
    self.fsm.vars.target_pos_odom = tfm.transform(TARGET_POS, "conveyor_pose",
                                                  "odom")
end
