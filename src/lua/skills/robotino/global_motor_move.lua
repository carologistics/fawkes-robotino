...
depends_interfaces = {{v = "motor", type = "MotorInterface", id = "Robotino"}}
...
function invalid_input()
    if fsm.vars.ori and math.abs(fsm.vars.ori) > math.pi then return true end
    return false
end
...
function pose_ok()
    ...
    local pose_ok = math.abs(dist.x) <= TOLERANCE.x and math.abs(dist.y) <=
                        TOLERANCE.y
...
function trans_error()
    return math.abs(fsm.vars.bl_target.x) > mm_tolerance.x or
               math.abs(fsm.vars.bl_target.y) > mm_tolerance.y
end
...
fsm:define_states{
    ...
    {"STARTPOSE", JumpState},
    {
        ...
        SkillJumpState,
        skills = {{motor_move}},
        final_to = "WAIT_PLAUSIBLE_TARGET",
        fail_to = "FAILED"
    },
...
fsm:add_transitions{
    {"INIT", "WAIT_PLAUSIBLE_POSE", cond = true}, {
        ...
        "WAIT_PLAUSIBLE_POSE",
        "FAILED",
        cond = "vars.pose_tries >= MAX_POSE_TRIES",
        desc = "no plausible pose!"
    },
    {
        "STARTPOSE",
        "TURN",
        cond = "vars.puck and vars.bl_target.x < -TOLERANCE.x"
    }, {"STARTPOSE", "DRIVE", cond = trans_error}, {
        "STARTPOSE",
        "TURN_BACK",
        cond = "vars.turn == true and math.abs(vars.bl_target.ori) > TOLERANCE.ori"
    }, {"STARTPOSE", "FINAL", cond = true}, {
        "WAIT_PLAUSIBLE_TARGET",
        "DRIVE",
        cond = target_dist_ok,
        desc = "target dist OK"
    }, {
...
function INIT:init()
    ...
    if type(self.fsm.vars.place) == "string" then
        ...
        if node then
            x = node:x()
            y = node:y()
            ori = node:has_property("orientation") and
                      node:property_as_float("orientation") or
                      self.fsm.vars.startpos.ori
        end
    end
...
function STARTPOSE:init()
    ...
    printf("%f, %f, %f", self.fsm.vars.bl_target.x, self.fsm.vars.bl_target.y,
           self.fsm.vars.bl_target.ori)
end
...
function TURN:init()
    ...
    self.args["motor_move"].ori = math.atan2(self.fsm.vars.bl_target.y,
                                             self.fsm.vars.bl_target.x)
    printf("atan ori: %f", self.args["motor_move"].ori)
    self.args["motor_move"].vel_rot = 1.2
    self.args["motor_move"].tolerance = mm_tolerance
end

function WAIT_PLAUSIBLE_TARGET:init() self.fsm.vars.target_tries = 0 end
