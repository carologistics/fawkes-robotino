...
function conveyor_ready(self)
  ...
  if if_conveyor:euclidean_fitness() > 0 then
    return true
  else
    printf("mps_approach failed: euclidean_fitness is %f, dist to object in front is %f. I don't drive with this visibility history or this far without collision avoidance", if_conveyor:euclidean_fitness(), self.fsm.vars.pose.x)
    return false
  end
end
...
function APPROACH_CONVEYOR:init()
  ...
  self.args["motor_move"] = {x = x_goal, vel_trans = 0.05}
end
