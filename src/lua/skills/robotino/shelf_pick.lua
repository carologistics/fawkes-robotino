...
function find_ll_direction(lines)
    ...
    if closest_ll == -1 then
        ...
        direction = {
            x = lines[closest_ll]:line_direction(0),
            y = lines[closest_ll]:line_direction(1)
        }
        length_direction = math.sqrt(math.pow(direction.x, 2) +
                                         math.pow(direction.y, 2))
        return {
            x = direction.x / length_direction,
            y = direction.y / length_direction
        }
    end
end
...
function is_grabbed()
    ...
    if robotino_sensor:is_digital_in(0) == false and
        robotino_sensor:is_digital_in(1) == true then -- white cable on DI1 and black on DI2
        return true
    else
        return false
    end
...
function pose_gripper_offset(x, y, z)
    local target_pos = {x = x, y = y, z = z, ori = {x = 0, y = 0, z = 0, w = 0}}
    local tmp = {x = 0, y = 0, z = 0, ori = {x = 0, y = 0, z = 0, w = 0}}
    ...
    local gripper_home_rel = tfm.transform6D(gripper_rel, "gripper",
                                             "gripper_home")

    -- Clip to axis limits
    return {
        x = math.max(0, math.min(gripper_home_rel.x, fsm.vars.x_max)),
        y = math.max(-fsm.vars.y_max / 2,
                     math.min(gripper_home_rel.y, fsm.vars.y_max / 2)),
...
fsm:define_states{
    ...
    },
    {
        ...
        SkillJumpState,
        skills = {{motor_move}},
        final_to = "RESET_GRIPPER",
        fail_to = "FAILED"
    },
    {
        "RESET_GRIPPER",
        SkillJumpState,
        skills = {{reset_gripper}},
        final_to = "CHECK_PUCK",
        fail_to = "FAILED"
    },
...
function INIT:init()
    ...
    if config:exists("/skills/shelf_pick/gripper_adjust_z_distance") then
        gripper_adjust_z_distance = config:get_float(
                                        "/skills/shelf_pick/gripper_adjust_z_distance")
    end
    ...
    self.fsm.vars.left_slot_y_offset = config:get_float(
                                           "/skills/shelf_pick/left_slot_y_offset")
    self.fsm.vars.middle_slot_y_offset = config:get_float(
                                             "/skills/shelf_pick/middle_slot_y_offset")
    self.fsm.vars.right_slot_y_offset = config:get_float(
                                            "/skills/shelf_pick/right_slot_y_offset")
...
function GOTO_SHELF:init()
    ...
    local target_pos_cp = {
        x = gripper_adjust_x_distance,
        y = 0.0,
        z = 0.0,
        ori = {x = 0, y = 0, z = 0, w = 0}
    }

    local target_pos_odom = tfm.transform6D(target_pos_cp, "conveyor_pose",
                                            "odom")
    ...
    self.fsm.vars.target_pos_odom = tfm.transform6D(target_pos_bl, "base_link",
                                                    "odom")
...
function MOVE_ABOVE_PUCK:init()
    local grip_pos = tfm.transform6D(self.fsm.vars.target_pos_odom, "odom",
                                     "gripper")
...
function GRAB_PRODUCT:init() self.args["gripper_commands"].command = "CLOSE" end

function LEAVE_SHELF:init() self.args["motor_move"].x = -0.2 end
