...
depends_interfaces = {
    ...
    {v = "line8", type = "LaserLineInterface", id = "/laser-lines/8"}, {
        v = "line1_avg",
        type = "LaserLineInterface",
        id = "/laser-lines/1/moving_avg"
    }, {
        v = "line2_avg",
        type = "LaserLineInterface",
        id = "/laser-lines/2/moving_avg"
    }, {
        v = "line3_avg",
        type = "LaserLineInterface",
        id = "/laser-lines/3/moving_avg"
    }, {
        v = "line4_avg",
        type = "LaserLineInterface",
        id = "/laser-lines/4/moving_avg"
    }, {
        v = "line5_avg",
        type = "LaserLineInterface",
        id = "/laser-lines/5/moving_avg"
    }, {
        v = "line6_avg",
        type = "LaserLineInterface",
        id = "/laser-lines/6/moving_avg"
    }, {
        v = "line7_avg",
        type = "LaserLineInterface",
        id = "/laser-lines/7/moving_avg"
    }, {
        v = "line8_avg",
        type = "LaserLineInterface",
        id = "/laser-lines/8/moving_avg"
...
documentation =
    [==[Align precisely at the given coordinates, relative to the center of an MPS
...
local TURN_MOVES = {{ori = math.pi}, {ori = -math.pi / 2}, {ori = -math.pi}}
local MAX_TRIES = 8

function tag_visible(vis_hist)
    local tag_iface = nil
    if fsm.vars.tag_id then
        tag_iface = tag_utils.iface_for_id(fsm.vars.tags, tag_info,
                                           fsm.vars.tag_id)
    else
        local min_dist = 1000
        for i = 0, 15 do
            if tag_info:tag_id(i) ~= 0 and
                fsm.vars.tags[i + 1]:visibility_history() > vis_hist and
                fsm.vars.tags[i + 1]:translation(2) < min_dist then
...
function want_search() return fsm.vars.search_idx <= MAX_TRIES end
...
fsm:define_states{
    ...
    closure = {
        ...
        MIN_VIS_HIST_TAG = MIN_VIS_HIST_TAG,
        want_search = want_search,
        line_visible = line_visible
    },
    {"INIT", JumpState},
    {
        "GRIPPER_PRE_CONVEYOR",
        SkillJumpState,
...
fsm:add_transitions{
    ...
    {"INIT", "GRIPPER_PRE_CONVEYOR", cond = true}, {
        "CHECK_TAG",
        "MATCH_LINE",
        cond = "tag_visible(MIN_VIS_HIST_TAG)",
        desc = "found tag"
    }, {"CHECK_TAG", "FIND_TAG", timeout = 2, desc = "no tag"}, {
        "FIND_TAG",
        "SEARCH_LINES",
        cond = "want_search() and #vars.interesting_lines > 0",
        desc = "search 4 tag"
    }, {
        "FIND_TAG",
        "MATCH_LINE",
...

    {
        ...
        "MATCH_LINE",
        "ALIGN_FAST",
        cond = "vars.matched_line and tag_visible(MIN_VIS_HIST_TAG)"
...
function match_line(tag, lines)
    ...
    if tag and tag:visibility_history() >= MIN_VIS_HIST_TAG then
        ...
        for k, line in pairs(lines) do
            ...
            local dist = math.vec_length(tag_laser.x - line_center.x,
                                         tag_laser.y - line_center.y)
            if line:visibility_history() >= MIN_VIS_HIST_LINE and dist <
                LINE_MATCH_TOLERANCE and dist < min_dist then
                min_dist = dist
                matched_line = line
...
function get_interesting_lines(lines)
    ...
    local bad_tags =
        tag_utils.bad_tags(fsm.vars.tags, tag_info, fsm.vars.tag_id)
    ...
    for k, v in pairs(fsm.vars.lines_visited) do
        if v > max_num_visited then max_num_visited = v end
    end
    ...
    for k, line in pairs(good_lines) do
        if line then
            ...
            if line:visibility_history() >= MIN_VIS_HIST_LINE_SEARCH and
                line:length() >= LINE_LENGTH_MIN and line:length() <=
                LINE_LENGTH_MAX and fsm.vars.lines_visited[line:id()] <
                max_num_visited and
                math.min(line:end_point_1(0), line:end_point_2(0)) <=
                LINE_XDIST_MAX then
...
function SEARCH_LINES:init()
    ...
    for i, l in ipairs(self.fsm.vars.interesting_lines) do
        local center = tfm.transform(llutils.center(l, 0), "/base_laser",
                                     "/base_link")
    ...
    self.fsm.vars.lines_visited[chosen_line:id()] =
        self.fsm.vars.lines_visited[chosen_line:id()] + 1
...
function TURN_AROUND:init()
    ...
    self.args["motor_move"] = TURN_MOVES[math.mod(self.fsm.vars.search_idx,
                                                  #TURN_MOVES) + 1]
    self.fsm.vars.search_idx = self.fsm.vars.search_idx + 1
...
function MATCH_LINE:init() self.fsm.vars.align_attempts = 0 end

function MATCH_LINE:loop()
    local tag = tag_utils.iface_for_id(self.fsm.vars.tags, tag_info,
                                       self.fsm.vars.tag_id)
    self.fsm.vars.matched_line = match_line(tag, self.fsm.vars.lines)
end

function SEARCH_TAG_LINE:init()
    self.fsm.vars.approached_tag = true
    local frame = tag_utils.frame_for_id(self.fsm.vars.tags, tag_info,
                                         self.fsm.vars.tag_id)

    if frame then
        self.args["motor_move"] = {x = 0.4, ori = math.pi, frame = frame}
    else
        ...
        --
        self.args["motor_move"] = {frame = "LOST_TAG_FRAME"}
    end
end
...
function MATCH_AVG_LINE:loop()
    local tag = tag_utils.iface_for_id(fsm.vars.tags, tag_info,
                                       self.fsm.vars.tag_id)
    if tag then
        ...
        if matched_line then
            ...
            local p_tag = tfm.transform6D({
                ...
                ori = fawkes.tf.create_quaternion_from_yaw(
                    matched_line:bearing())
            }, matched_line:frame_id(), "/odom")
            if p_tag then self.fsm.vars.p_tag = p_tag end
        end
    end
end
...
function ALIGN_PRECISE:init()
    ...
    if self.fsm.vars.p_tag then
        printf("p_tag: %f %f %f", self.fsm.vars.p_tag.x, self.fsm.vars.p_tag.y,
        ...
        print(
            "WARNING: lost laser line before precise alignment. Continuing with odometry only.")
        self.args["motor_move"].y = self.fsm.vars.y
    end
end

function ALIGN_TURN:init() self.args["motor_move"] = {ori = self.fsm.vars.ori} end
