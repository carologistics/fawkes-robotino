...
fsm:define_states{
    export_to = _M,
    closure = {navgraph = navgraph, is_grabbed = is_grabbed},
    {"INIT", JumpState},
    {
...
fsm:add_transitions{
    {"INIT", "FAILED", cond = "not navgraph", desc = "navgraph not available"},
    {
        ...
        desc = "At mps, skip drive_to_local"
...
function INIT:init()
    ...
    if self.fsm.vars.side == nil then self.fsm.vars.side = "input" end
end
...
function DRIVE_TO_MACHINE_POINT:init()
    ...
    if self.fsm.vars.slide then option = "SLIDE" end
