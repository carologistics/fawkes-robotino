...
(defrule net-send-BeaconSignal-with-pose
  ...
  (pb-set-field ?beacon-pose "ori" (nth$ 3 ?pos))
  (bind ?pose-time (pb-field-value ?beacon-pose "timestamp"))
  (pb-set-field ?pose-time "sec" (nth$ 1 ?now))
  (pb-set-field ?pose-time "nsec" (* (nth$ 2 ?now) 1000))
  (pb-set-field ?beacon-pose "timestamp" ?pose-time)
