  ...
  (foreach ?m (pb-field-list ?p "machines")
    ...
    (if (eq ?m-type RS) then
      (do-for-fact ((?rs ring-station)) (eq ?rs:name ?m-name)
        (if (eq 0 (length$ ?rs:availablecolors)) then
          (bind ?rs-fact ?rs)
          (progn$ (?rc (pb-field-list ?m "ring_colors"))
            (bind ?rs-fact (synced-add-to-multifield ?rs-fact availablecolors (utils-remove-prefix ?rc "RING_")))
          )
        )
      )
    )
...
(defrule wm-insert-cap-into-cs-final
  ...
  (task (name fill-cap|clear-cs|load-cs))
...
(defrule wm-insert-product-into-rs-final
  ...
  (task (name add-ring-one|add-ring-two|add-ring-three) (state running))
  ?mf <- (machine (name ?mps) (loaded-id 0) (produced-id 0))
...
(defrule wm-insert-base-into-rs-slide-final
  ...
  (task (name add-base-to-slide-one|add-base-to-slide-two|add-base-to-slide-three) (state running))
  ?rsf <- (ring-station (name ?mps) (bases-loaded ?bl))
  ?hf <- (holding ?product-id)
