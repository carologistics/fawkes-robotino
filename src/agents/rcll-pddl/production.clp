...
(defrule prod-lock-position-not-holding
  ...
  (not (stn-action (id ?oid&:(< ?oid ?id)) (name lock-position) (state pending)))
  (holding NONE)
  (not (wait-for-lock (state use)))
  (stn-sync (state generated))
  =>
  (printout t "PROD: Locking " ?to " for action " ?action crlf)
...
(defrule prod-start-action-after-lock
  ...
  (state IDLE|WAIT_AND_LOOK_FOR_ALTERATIVE)
  (team-color ?team-color&~nil)
  ?sa <- (stn-action (id ?id) (name lock-position) (state pending)
              (cond-actions) (opts ?r ?action ?to))
  (not (stn-action (id ?oid&:(< ?oid ?id)) (name lock-position) (state pending)))
...
(defrule prod-lock-release
  ...
  (state IDLE|WAIT_AND_LOOK_FOR_ALTERATIVE)
  (team-color ?team-color&~nil)
  ?sal <- (stn-action (id ?lock-id) (name lock-position) (state running)
                      (active-robot ?ar&:(eq ?ar (sym-cat ?*ROBOT-NAME*))) (opts ?r ?action ?to))
  ?sa <- (stn-action (id ?id) (name unlock-position) (state pending) 
...
;  (holding NONE)
;  ?sal <- (stn-action (id ?lock-id) (name lock-position) (state running)
;                      (active-robot ?ar&:(eq ?ar ?*ROBOT-NAME*)))
;  ?sa <- (stn-action (id ?id) (name lock-change) (state pending) 
;              (cond-actions ?ca&:(member$ ?lock-id ?ca)) (opts ?r ?old-action ?action ?to))
;  (wait-for-lock (res ?to) (state use))
;  ?lock <- (stn-action-lock ?lock-id ?old-action ?to)
;  =>
...
;  (retract ?lock)
;  (assert (stn-action-lock ?lock-id ?action ?to))
;)
...
(defrule prod-pick-wp-from-bs
  ...
  (stn-action (id ?prod-id) (name prod-at-cs) (cond-actions $?pca&:(member$ ?id ?pca)))
  (stn-action (id ?del-id) (name deliver-c0) (cond-actions $?dca&:(member$ ?prod-id ?dca))
              (opts ?any1 ?any2 ?any3 ?any4 ?any5 ?any6 ?order))
...
(defrule prod-deliver-c0
  ...
  (holding ?base)
  ?sal <- (stn-action (id ?lock-id) (name lock-position) (state running)
                      (active-robot ?ar&:(eq ?ar (sym-cat ?*ROBOT-NAME*))) (opts ?r DELIVER-C0 ?to))
  ?sa <- (stn-action (id ?id) (name deliver-c0) (state pending) (cond-actions $?ca&:(member$ ?lock-id ?ca))
                     (opts ?r ?mps ?to ?product ?base-col ?cap-col ?order))
  (machine (mtype DS)
  ...
  (synced-modify ?sa state running)
  (bind ?task-id (random-id))
  (assert (task (name deliver-c0) (id ?task-id) (state proposed) (stn-action ?id)
		        (steps (create$ (+ ?task-id 1)))
		        (priority ?*PRIORITY-PRODUCE-C0*))
    (step (name insert) (id (+ ?task-id 1))
		  (task-priority ?*PRIORITY-PRODUCE-C0*)
