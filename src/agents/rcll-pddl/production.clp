...
(defrule prod-lock-position-not-holding
  ...
  (not (locked-resource (resource ?to) (agent ?agent&:(eq ?agent ?*ROBOT-NAME*))))
...
(defrule prod-move-to-position-empty
  ...
  ?sa <- (stn-action (id ?id)
                     (name ?task-name&:(or (eq ?task-name move-to-position-empty)
                                           (eq ?task-name move-to-position-holding)))
                     (state pending)
