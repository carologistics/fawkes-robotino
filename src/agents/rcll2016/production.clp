...
(defrule prod-prefill-ring-station
  ...
  (not (and (task (name fill-rs) (state rejected) (id ?rej-id))
            (step (name insert) (id ?rej-st&:(eq ?rej-st (+ ?rej-id 3))) (machine ?rs))))
...
(defrule prod-produce-c0
  ...
  (not (and (task (name produce-c0) (state rejected) (id ?rej-id))
            (step (name insert) (id ?rej-st&:(eq ?rej-st (+ ?rej-id 4))) (machine ?cs))))
...
(defrule prod-remove-product-with-expired-order-from-rs
  ...
  (not (and (task (name clear-rs) (state rejected) (id ?rej-id))
            (step (name insert) (id ?rej-st&:(eq ?rej-st (+ ?rej-id 1))) (machine ?rs))))
