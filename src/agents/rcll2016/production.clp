...
(deffunction smt-create-data (?robots ?addrobots ?machines ?orders ?capcolors ?ringdata ?workingpiece)
	...
	(foreach ?c ?capcolors
		(pb-add-list ?p "orders" ?c)
	)
	(foreach ?r ?ringdata
		(pb-add-list ?p "machines" ?r)
	)
	(foreach ?w ?workingpiece
		(pb-add-list ?p "orders" ?w)
	)
...
(deffunction smt-create-capcolors (?ccolor ?team-color)
	(bind ?o (pb-create "llsf_msgs.Order"))
	(switch ?ccolor
...
(deffunction smt-create-ringcolors-baseloaded (?rcolor ?baseloaded ?team-color)
	(bind ?m (pb-create "llsf_msgs.Machine"))
	(bind ?rlist (create$))
  (switch ?rcolor
  ...
  (pb-set-field ?m "loaded_with" ?baseloaded) 
	(return ?m)
)
...
(deffunction smt-create-get-product (?pid ?team-color)
  (bind ?rv (create$))
  (do-for-all-facts ((?p product)) TRUE
		(if (eq ?p:id ?pid) then
			(bind ?rv (append$ ?rv (smt-create-order-productcolors ?p:rings ?p:cap ?p:base ?team-color)))
		)
  )
	(return ?rv)
)
	
(deffunction smt-create-product-at-machine-output (?team-color)
	(bind ?rv (create$))
  (do-for-all-facts ((?m machine)) (eq ?m:team ?team-color)
		(if (eq ?m:state READY-AT-OUTPUT) then		 					
		  (bind ?rv (append$ ?rv (smt-create-get-product ?m:produced-id ?team-color)))
	  )
	)
	(return ?rv)
)

(deffunction smt-create-capstationdata (?team-color)
	(bind ?rv (create$))
  (do-for-all-facts ((?c cap-station)) (neq ?c:cap-loaded NONE)
		(bind ?rv (append$ ?rv (smt-create-capcolors ?c:cap-loaded ?team-color)))
	)
	(return ?rv)
)

(deffunction smt-create-ringstationdata (?team-color)
	(bind ?rv (create$))
  (do-for-all-facts ((?r ring-station)) (neq ?r:selected-color NONE)
		(bind ?rv (append$ ?rv (smt-create-ringcolors-baseloaded ?r:selected-color ?r:bases-loaded ?team-color)))
	)
	(return ?rv)
)

(defrule production-call-clips-smt
	(declare (salience ?*PRIORITY-CALL-CLIPS-SMT*))
  (phase PRODUCTION)
  (team-color ?team-color&CYAN|MAGENTA)
	(state IDLE)
	(not (plan-requested))
	...
	(bind ?p
	  (smt-create-data
	    ...
	    (smt-create-orders ?team-color)
		  (smt-create-capstationdata ?team-color)
			(smt-create-ringstationdata ?team-color)
			(smt-create-product-at-machine-output ?team-color)
	  )
	)
...
(defrule prod-prefill-cap-station
  ...
  (assert (task (name fill-cap) (id ?task-id) (state proposed)
		...
		(priority ?*PRIORITY-PREFILL-CS*)(robot ?*ROBOT-NAME*))
...
(defrule prod-remove-empty-base-from-cs
  ...
  (assert (task (name clear-cs) (id ?task-id) (state proposed)
		...
		(priority ?*PRIORITY-CLEAR-CS*)(robot ?*ROBOT-NAME*))
...
(defrule prod-prefill-ring-station
  ...
  (assert (task (name fill-rs) (id ?task-id) (state proposed)
		...
		(priority ?*PRIORITY-PREFILL-RS*)(robot ?*ROBOT-NAME*))
...
(defrule insert-unintentionally-holding-base-to-rs
  ...
  (assert (task (name fill-rs) (id ?task-id) (state proposed)
    (steps (create$ (+ ?task-id 1)))
    (priority ?*PRIORITY-PREFILL-RS-WITH-HOLDING-BASE*)(robot ?*ROBOT-NAME*))
	  (step (name insert) (id (+ ?task-id 1))
...
(defrule discard-unneeded-base
  ...
  (bind ?task-id (random-id))
 (assert (task (name discard-unknown) (id ?task-id) (state proposed)
    (steps (create$ (+ ?task-id 1)))
    (priority ?*PRIORITY-DISCARD-UNKNOWN*) (robot ?*ROBOT-NAME*))
...
(defrule prod-clear-bs
  ...
  (bind ?task-id (random-id))
  (assert (task (name clear-bs) (id ?task-id) (state proposed)
    (steps (create$ (+ ?task-id 1)))
    (priority ?*PRIORITY-CLEAR-BS*)(robot ?*ROBOT-NAME*))
    (step (name get-output) (id (+ ?task-id 1))
...
(defrule prod-produce-c0
  ...
  (assert (task (name produce-c0) (id ?task-id) (state proposed)
    ...
    (priority ?*PRIORITY-PRODUCE-C0*)(robot ?*ROBOT-NAME*))
    (step (name drive-to) (id (+ ?task-id 1))
...
(defrule prod-add-first-ring
  ...
  (assert (task (name add-first-ring) (id ?task-id) (state proposed)
    (steps (create$ (+ ?task-id 1) (+ ?task-id 2) (+ ?task-id 3) (+ ?task-id 4) (+ ?task-id 5) (+ ?task-id 6)))
    (priority ?*PRIORITY-ADD-FIRST-RING*)(robot ?*ROBOT-NAME*))
    (step (name drive-to) (id (+ ?task-id 1))
...
(defrule prod-add-additional-ring
  ...
  (assert (task (name add-additional-ring) (id ?task-id) (state proposed)
    ...
    (priority ?*PRIORITY-ADD-ADDITIONAL-RING*)(robot ?*ROBOT-NAME*))
    (step (name drive-to) (id (+ ?task-id 1))
...
(defrule prod-add-additional-ring-with-waiting
  ...
  (assert (task (name add-additional-ring) (id ?task-id) (state proposed)
    (steps (create$ (+ ?task-id 1) (+ ?task-id 2) (+ ?task-id 3) (+ ?task-id 4) (+ ?task-id 5)))
    (priority ?*PRIORITY-ADD-ADDITIONAL-RING-WAITING*)(robot ?*ROBOT-NAME*))
    (step (name drive-to) (id (+ ?task-id 1))
...
(defrule prod-produce-cx
  ...
  (assert (task (name produce-cx) (id ?task-id) (state proposed)
    ...
    (priority ?*PRIORITY-PRODUCE-CX*)(robot ?*ROBOT-NAME*))
    (step (name get-output) (id (+ ?task-id 1))
...
(defrule prod-deliver
  ...
  (assert (task (name deliver) (id ?task-id) (state proposed)
    (steps (create$ (+ ?task-id 1) (+ ?task-id 2) (+ ?task-id 3) (+ ?task-id 4)))
    (priority ?*PRIORITY-DELIVER*)(robot ?*ROBOT-NAME*))
    (step (name drive-to) (id (+ ?task-id 1))
      (task-priority ?*PRIORITY-PRODUCE-CX*)
      (machine ?cs)
      (side OUTPUT))
...
(defrule prod-remove-product-with-expired-order-from-rs
  ...
  (assert (task (name clear-rs) (id ?task-id) (state proposed)
                (steps (create$ (+ ?task-id 1)))
                (priority ?*PRIORITY-CLEAR-RS*)(robot ?*ROBOT-NAME*))
    (step (name get-output) (id (+ ?task-id 1))
...
(defrule prod-find-missing-mps-exploration-catch-up
  ...
  (assert (task (name exploration-catch-up) (id ?task-id) (state proposed)
		(steps (create$ (+ ?task-id 1)))
		(priority ?*PRIORITY-FIND-MISSING-MPS*)(robot ?*ROBOT-NAME*))
