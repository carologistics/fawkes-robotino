...
(defrule prod-produce-P3-and-deliver
  ...
  (not (task (name produce-with-S0) (args $?args&:(subsetp ?args (create$ ?name))) (state rejected)))
  (holding NONE|S0)
  ...
  (assert (task (name produce-p3-and-deliver) (id ?task-id) (state proposed)
		(steps (create$ (+ ?task-id 1) (+ ?task-id 2) (+ ?task-id 3))) (current-step (+ ?task-id 1)))
	  (step (name get-s0) (id (+ ?task-id 1)))
          (step-args-get-s0 (id (+ ?task-id 1)) (task-priority ?*PRIORITY-PRODUCE-T5-AT-BEGIN*))
	  (step (name produce-at) (id (+ ?task-id 2)))
          (step-args-produce-at (id (+ ?task-id 2)) (machine ?name))
	  (step (name deliver) (id (+ ?task-id 3)))
          (step-args-deliver (id (+ ?task-id 3)) (product-type P3))
	  (needed-task-lock (id ?task-id) (action BRING_S0) (place ?name))
	  ;TODO: set resource field automatically
  )
)
