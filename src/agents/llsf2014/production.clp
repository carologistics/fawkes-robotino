...
(defrule prod-deliver-P3
  ...
  (assert (proposed-task (name pick-and-deliver) (args (create$ ?name)) (priority ?*PRIORITY-DELIVER-P3*))
  )
)
...
(defrule prod-deliver-P1-P2
  ...
  (assert (proposed-task (name pick-and-deliver) (args (create$ ?name))  (priority ?*PRIORITY-DELIVER-P1P2*))
  )
)
...
(defrule prod-load-T2-with-S1
  ...
  (machine (mtype T2) (loaded-with $?l&~:(member$ S1 ?l))
	   (incoming $?i&~:(member$ BRING_S1 ?i)) (name ?name-T2) (produced-puck NONE))
...
(defrule prod-load-T3_T4-with-S1
  ...
  (machine (mtype T3|T4) (loaded-with $?l&~:(member$ S1 ?l))
	   (incoming $?i&~:(member$ BRING_S1 ?i)) (name ?name-T3_T4) (produced-puck NONE))
