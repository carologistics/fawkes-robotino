...
##########################################################################
#
#  ImageAugmentation.py: Augment Data from the training dataset using the Coco2017 test data
#
...
seq = iaa.Sequential(
    [
        ...
        iaa.Sometimes(
            0.5,
            iaa.BlendAlphaBoundingBoxes(
                None,
                background=iaa.BlendAlphaRegularGrid(
                    nb_rows=(7, 30),
                    nb_cols=(6, 24),
...
for image_file in entries:
    ...
    for i in range(num_copy):
        ...
        aug_pixels = np.where((image_aug[:, :, 0] == 0) & (image_aug[:, :, 1] == 0) & (image_aug[:, :, 2] == 0))
