...
parser.add_argument("--nbots", type=int, default=3, help="number of active fawkes instances, ignore when central")
parser.add_argument("--central", type=bool, default=False, help="agent in central mode")
parser.add_argument("--path", type=str, default="", help="path to the log files")
parser.add_argument(
    "--postfix",
    type=str,
    default="_latest.log",
    ...
    with open(filename, "r") as reader:
        for line in reader.readlines():
            if "(goal (id" in line and "==>" in line:
                parsed = parse(
                    "D {time} {} ==> {} (goal (id {id:S}) (class {class:S}) "
                    + "(type {type:S}) (sub-type {subtype:S}) (parent {parent:S}) "
                    + "(mode {mode:S}) (outcome {outcome:S}){}(params{params}){}",
                    line,
                ).named
...
print("deliveries")
print(
    datasets["production_points"][datasets["production_points"]["reason"].str.contains("Delivered item")][
        ["game-time", "reason"]
    ]
)
print("orders")
