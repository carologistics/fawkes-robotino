...
parser = argparse.ArgumentParser(description=textwrap.dedent(header), formatter_class=argparse.RawTextHelpFormatter)
parser.add_argument(
    "--baseline-prefix",
    type=str,
    help="prefix string of directories containing the baseline results, e.g for directories baseline1,"
    + ' baseline2, etc. this would be "baseline"',
    default="baseline",
)
parser.add_argument(
    "--experiment-prefix",
    type=str,
    help="prefix string of directories containing the experimental results, e.g for directories"
    + ' experiment1, experiment2, etc. this would be "experiment"',
    default="experiment",
)
parser.add_argument(
    "--path", type=str, required=True, help="path of the root directory containing all the runs of the given experiment"
)
parser.add_argument("--game_time", type=int, default=1200, help="number of seconds in the game")
...
def print_summary(data, operation, title):
    ...
    if operation is None:
        ...
        print(
            "Deliveries:     ",
            operation(data["baseline"]["deliveries"]),
            ":",
            operation(data["experiment"]["deliveries"]),
        )
...
def plot_summary(plt, grid_pos, data, operation, title):
    ...
    ax.text(
        0,
        0.4,
        "{:4.2f}:{:4.2f}".format(operation(data["baseline"]["points"]), operation(data["experiment"]["points"])),
        weight="bold",
    )
    ...
    ax.text(
        ...
        0.4,
        "{:4.2f}:{:4.2f}".format(
            operation(data["baseline"]["deliveries"]), operation(data["experiment"]["deliveries"])
        ),
        weight="bold",
    )
    ...
    ax.text(
        0.66,
        0.4,
        "{:4.2f}:{:4.2f}".format(operation(data["baseline"]["late"]), operation(data["experiment"]["late"])),
        weight="bold",
    )
...
if consistency_check():
            ...
            "runs": [
                {
                    ...
                    "meta": pd.read_csv(args.path + "/" + name + "/" + "game-export_meta.csv"),
                }
                for name in next(os.walk(args.path))[1]
                if args.baseline_prefix in name
            ],
        },
        "experiment": {
            "points": [],
            ...
            "runs": [
                ...
                for name in next(os.walk(args.path))[1]
                if args.experiment_prefix in name
            ],
        },
    }
    ...
    for run in results["baseline"]["runs"]:
        ...
        results["baseline"]["deliveries"].append(
            len(run["points"].loc[run["points"]["reason"].str.contains("Delivered item")].index)
        )
    ...
    for run in results["experiment"]["runs"]:
        results["experiment"]["points"].append(
            int(run["meta"].loc[run["meta"]["key"] == "points:cyan"]["value"].iloc[0])
        )
        results["experiment"]["deliveries"].append(
            len(run["points"].loc[run["points"]["reason"].str.contains("Delivered item")].index)
        )
    ...
    []
    levels = np.tile(
        [x for i in range(1, int(len(timepoints_start) / 2) + 1) for x in (i, -i)],
        int(np.ceil(len(timepoints_start) / ((len(timepoints_start) / 2 + 1) * 2))),
    )[: len(timepoints_start)]
    ...
    ax.plot([0, args.game_time], [0, 0], "-", color="k", markerfacecolor="w")  # Baseline and markers on it.
    ...
    for run in results["baseline"]["runs"]:
        ax.plot(
            [0, args.game_time], [num_lines, num_lines], "-", color="k", markerfacecolor="w"
    ...
    ax.text(
        0,
        0.0,
        "Contains overviews of the key performance statistics w.r.t to a\nstandard game of the RCLL: "
        + "points, number of deliveries and number\nof late deliveries. Read (value baseline):(value experiment)",
        fontsize=12,
    )
    ...
    for index, row in results["baseline"]["runs"][0]["orders"].iterrows():
        ...
        ax.add_patch(
            patches.Rectangle(
                (left_spacing, 0),
                spacing_step - 4 * (spacing_step / 10),
                0.2,
                linewidth=1,
                facecolor=row["base-color"][5:],
            )
        )
        if type(row["ring-colors"]) is not float:
            for color in row["ring-colors"].split(" "):
                ax.add_patch(
                    patches.Rectangle(
                        (left_spacing, height_spacing),
                        spacing_step - 4 * (spacing_step / 10),
                        0.1,
                        linewidth=1,
                        facecolor=color[5:],
