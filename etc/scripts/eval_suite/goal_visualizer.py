...
def plot_goals(goals, ax, ps, pe, title):
    ...
    for index, row in goals.iterrows():
        ...
        ax.barh(
            ...
            edgecolor=edgecolor,
            align="center",
            left=time_string_diff(ps, row["dispatched"]),
            color=goal_color_map[row["class"]],
        )
        if row["outcome"] == "UNKNOWN":
            ax.barh(
                0,
                duration,
                hatch="//",
                edgecolor=(0.2, 0.2, 0.8, 0.2),
                align="center",
                left=time_string_diff(ps, row["dispatched"]),
                color=(0, 0, 0, 0),
            )
        if duration > 30:
            plt.text(time_string_diff(ps, row["dispatched"]) + 1, 0.14, row["id"], fontsize="4", color=(1, 1, 1))
...
fg = df.loc[
    (df["dispatched"].notna())
    & (df["subtype"] == "SIMPLE")
    & ~(df["class"].isin(["SEND-BEACON", "EXPIRE-LOCKS", "SPAWN-WP", "PROCESS-MPS", "REFILL-SHELF", "MOVE-OUT-OF-WAY"]))
